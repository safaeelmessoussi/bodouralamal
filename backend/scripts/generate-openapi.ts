...
const document = {
  ...
  paths: {
    ...
    '/sessions/{id}/content': {
      post: op('Link content to a session', 'TD-3.12, §4.9. Body: `{ content_id }` — **strictly** that key. The link is **its own row**, so the returned `id` addresses the association rather than the file. Linking the same content twice is `409 DUPLICATE`; a previously unlinked pair relinks rather than creating a second row. **A linked content item is one of the things that makes a Session protected** (§4.4, R43.6) — a schedule edit will leave this occurrence alone and report it, rather than rewriting it — so this endpoint changes what a later schedule edit is allowed to do.', { '201': 'Linked.', '400': `${ENVELOPE} VALIDATION_FAILED — a missing or malformed content_id, or an unknown key.`, '401': ENVELOPE, '403': ENVELOPE, '404': `${ENVELOPE} NOT_FOUND for an unknown session, an unknown content item, or a session the caller does not staff.`, '409': `${ENVELOPE} DUPLICATE — already linked.` }),
    },
