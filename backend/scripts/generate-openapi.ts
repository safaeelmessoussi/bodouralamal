...
const document = {
  ...
  paths: {
    ...
    '/calendar': {
      get: op('Read the calendar', 'The one **public** read in the system (TD-3.4): an anonymous visitor receives the **public tier only**. One unified grid over the recurring **Group** timetable and the one-off **Event** exception layer, expanded across `from`..`to` (max 366 days — an unbounded range would expand every recurrence forever). All five recurrence types are supported; **biweekly-alternating** is week-on/week-off. **Visibility is resolved server-side from the caller\'s live roles, never from a parameter** (§4.4): a Pending account sees exactly what an anonymous visitor sees; an approved Student or Parent adds the private tier, which is deliberately **not** filtered by the student\'s own branch (Risk R-6); a Teacher additionally sees Hidden events whose scope intersects their assigned groups; **all Admins see Hidden regardless of branch scope** while their Private tier **is** branch-scoped; Super Admins see everything. A group timetable is not public information, so an anonymous caller receives no groups. When scoped to a branch, nothing before its `operational_start_date` is returned (§4.4). Every occurrence also carries the **decorative Hijri overlay** (`hijri_date`, `hijri_month_ar`) with the Super-Admin day-offset (−2…+2) **already applied server-side**, so `DualDateDisplay` renders it without re-deriving anything — it is a label, and nothing in scheduling or recurrence reads it back (§4.4, §5.7). Dates are `YYYY-MM-DD` and times `HH:MM`, local wall-clock (TD-11) — a 09:00 class stays at 09:00 across Morocco\'s Ramadan DST shift (§19.2).', { '200': 'Occurrences, ordered by date then time then id.', '400': `${ENVELOPE} VALIDATION_FAILED — missing/invalid dates, an inverted range, or one longer than 366 days.`, '401': `${ENVELOPE} AUTH_REQUIRED only when a token IS supplied but does not verify; an absent token is anonymous, not an error.` }),
    },
    '/events': {
    ...
    '/admin/groups': {
      get: op('List groups', 'Operational data (Revision 26): Admins see groups in their own branches, Super Admins all. Ordered by day then start time.', { '200': 'Groups, with `start_time`/`end_time` as local wall-clock `HH:MM` (TD-11).', '401': ENVELOPE, '403': ENVELOPE }),
    ...
    '/admin/groups/{id}/roster': {
      get: op('List a group\'s roster', '§5.6 Group Enrollment. Admin within branch scope or Super Admin (TD-2).', { '200': 'Enrolled students, ordered by Arabic name.', '401': ENVELOPE, '403': ENVELOPE, '404': `${ENVELOPE} NOT_FOUND for a group out of scope or nonexistent.` }),
