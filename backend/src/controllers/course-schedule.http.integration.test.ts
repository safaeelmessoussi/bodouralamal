...
describe("the response is an explicit contract DTO (§16.2)", () => {
  it("POST returns the schedule plus what it did to the timetable", async () => {
    ...
    expect(Object.keys(res.body.schedule as object).sort()).toEqual(
      SCHEDULE_KEYS,
    );
    expect(Object.keys(res.body.materialization!).sort()).toEqual(
    ...
    expect(Array.isArray(res.body.materialization!["protected_sessions"])).toBe(
      true,
    );
  ...
  it("paginates per TD-10", async () => {
    const res = await call(
      "GET",
      "/admin/course-schedules?page=1&page_size=1",
      superAdmin,
    );
...
describe("the write boundary refuses what would re-point history", () => {
  ...
  it("refuses to change the subject, target, branch or academic year on PATCH", async () => {
    ...
    for (const move of [
      ...
      const res = await call(
        "PATCH",
        `/admin/course-schedules/${(created.body.schedule as { id: string }).id}`,
        superAdmin,
        {
  ...
  it("TD-15: a stale version is a 409, not a silent overwrite", async () => {
    ...
    expect((first.body.schedule as { start_time: string }).start_time).toBe(
      "20:00",
    );
  ...
  it("an edit reports what it left alone as well as what it rewrote", async () => {
    ...
    expect(Object.keys(patched.body.materialization!).sort()).toEqual(
      MATERIALIZATION_KEYS,
    );
...
describe("conflicts and the resolved roster", () => {
  it("reports a room clash found against MATERIALIZED sessions", async () => {
    ...
    if (preview.body.conflicts!.length > 0) {
      expect(Object.keys(preview.body.conflicts![0]!).sort()).toEqual(
        CONFLICT_KEYS,
      );
    }
  });
  ...
  it("resolves the audience live, and drops an un-enrolled student immediately", async () => {
    ...
    expect(Object.keys(before.body.students![0]!).sort()).toEqual([
      "name",
      "student_id",
    ]);
    ...
    expect(after.body.students!.map((s) => s.student_id)).not.toContain(
      studentA,
    );
