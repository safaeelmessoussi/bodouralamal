...
async function grant(userId: string, role: string, branch: string | null): Promise<void> {
...
describe('PATCH /admin/users/{id} — the person\'s own fields', () => {
  it('edits, and answers with the shape the list already renders', async () => {
    ...
    const list = await call('GET', `/admin/users?q=${encodeURIComponent(TAG)}`, superAdmin);
    expect(list.status).toBe(200);
    const row = (list.body as unknown as { data: Record<string, unknown>[] }).data.find(
      (r) => r['id'] === id,
    )!;
  ...
  it('records which fields changed, never their values', async () => {
    ...
    await grant(id, 'student', branchId);
    await call('PATCH', `/admin/users/${id}`, superAdmin, { version: 0, nickname: 'سرّي' });
...
describe('POST /admin/users/{id}/suspend — TD-1 Active → Suspended', () => {
  it('revokes every live session in the same transaction (TD-4.15)', async () => {
    ...
    const revocation = await prisma.auditLog.findFirst({
      where: { targetEntity: 'User', targetId: id, actionType: 'auth.token_revoked' },
    });
  ...
  it('refuses self-suspension', async () => {
    ...
    const res = await call('POST', `/admin/users/${superAdminId}/suspend`, superAdmin, {
      version: 0,
      reason: 'اختبار',
    });
    expect(res.status).toBe(409);
  ...
  it('reactivates, and leaves the sessions revoked', async () => {
    ...
    await grant(id, 'teacher', branchId);
    await call('POST', `/admin/users/${id}/suspend`, superAdmin, { version: 0, reason: 'اختبار' });

    const res = await call('POST', `/admin/users/${id}/reactivate`, superAdmin, { version: 1 });
    expect(res.status).toBe(200);
  ...
  it('will not reactivate a Rejected account — that status is terminal', async () => {
    ...
    await grant(id, 'student', branchId);
    const res = await call('POST', `/admin/users/${id}/reactivate`, superAdmin, { version: 0 });
    expect(res.status).toBe(409);
    expect(res.body.error?.details?.['reason']).toBe('INVALID_TRANSITION');
  });
});
...
describe('PUT /admin/users/{id}/roles — the complete assignment set', () => {
  ...
  it('revives a tombstoned assignment rather than colliding with the unique index', async () => {
    ...
    await grant(id, 'teacher', branchId);
    await call('PUT', `/admin/users/${id}/roles`, superAdmin, { assignments: [] });
    const res = await call('PUT', `/admin/users/${id}/roles`, superAdmin, {
      assignments: [{ role: 'teacher', branch_id: branchId }],
    });
    expect(res.status).toBe(200);
    expect(
      await prisma.userBranchRole.count({ where: { userId: id, branchId, deletedAt: null } }),
    ).toBe(1);
  });
  ...
  it('lets a Super Admin grant super_admin (Revision 22)', async () => {
    ...
    expect(res.status).toBe(200);
    expect((res.body.data!['roles'] as { role: string }[])[0]!.role).toBe('super_admin');
  });
  ...
  it('refuses an unknown role and an unknown branch', async () => {
    ...
    await grant(id, 'student', branchId);
    const unknownRole = await call('PUT', `/admin/users/${id}/roles`, superAdmin, {
      assignments: [{ role: 'headmistress', branch_id: null }],
    });
    expect(unknownRole.status).toBe(400);

    const unknownBranch = await call('PUT', `/admin/users/${id}/roles`, superAdmin, {
      assignments: [{ role: 'student', branch_id: '00000000-0000-4000-8000-000000000000' }],
    });
...
describe('who may manage users at all', () => {
  it('refuses a Teacher every verb', async () => {
    ...
    const teacher = bearer(teacherId, [{ role: 'teacher', branches: [branchId] }]);
    const target = await makeUser('هدف');
    await grant(target, 'student', branchId);

    expect((await call('PATCH', `/admin/users/${target}`, teacher, { version: 0 })).status).toBe(403);
    expect(
      (await call('POST', `/admin/users/${target}/suspend`, teacher, { version: 0, reason: 'x' }))
        .status,
    ).toBe(403);
    expect(
      (await call('PUT', `/admin/users/${target}/roles`, teacher, { assignments: [] })).status,
    ).toBe(403);
  });
...
/**
 * **`?eligible_for_level=` — the candidates a restricted Level can actually take**
 * (R27, added 2026-08-18).
 *
 * ## The defect this exists for
 *
 * The enrolment form offered every active account while the server refused a good
 * half of them: a `girls_only` Level rejects a male student **and a NULL sex**,
 * so the reader's only way to learn who was eligible was to pick somebody and
 * read a `GENDER_RESTRICTION`. Every other selector pair on that form already
 * narrowed (§14.4/R55); this one did not.
 *
 * ## Why the SERVER narrows it
 *
 * `sex` is not on the user contract and is deliberately not added. Staff placing
 * a beneficiary need the eligible **set**, not everybody's sex — the same
 * reasoning that keeps the student's own sex out of the `GENDER_RESTRICTION`
 * error (§4.10, BR-16). So the filter hands back a list and the fact behind it
 * never leaves the service.
 */
describe('GET /admin/users?eligible_for_level= — R27 narrowing', () => {
  ...
  beforeAll(async () => {
    ...
    girlsOnly = (
      await prisma.level.create({
        data: { name: `${TAG} مستوى للفتيات`, categoryId: category.id, genderRestriction: 'girls_only' },
      })
    ).id;
    unrestricted = (
      await prisma.level.create({
        data: { name: `${TAG} مستوى مفتوح`, categoryId: category.id, genderRestriction: 'any' },
      })
    ).id;
    girl = await makeUser('فتاة مؤهلة', 'active', 'female');
    boy = await makeUser('فتى', 'active', 'male');
    unknownSex = await makeUser('بلا جنس مسجّل', 'active', null);
  });

  const ids = async (query: string): Promise<string[]> => {
    const res = await call('GET', `/admin/users?page_size=100&${query}`, superAdmin);
    expect(res.status).toBe(200);
    return (res.body.data as unknown as Record<string, unknown>[]).map((u) => String(u['id']));
  };

  it('offers only the eligible sex for a restricted Level', async () => {
    const offered = await ids(`eligible_for_level=${girlsOnly}`);
    expect(offered).toContain(girl);
    expect(offered).not.toContain(boy);
  });

  it('excludes a NULL sex, exactly as enrolment refuses it', async () => {
    // The half that made this a real defect rather than an edge case: every
    // fixture person predating R27 had a NULL sex, so the form was offering
    // people no restricted Level could ever accept.
    const offered = await ids(`eligible_for_level=${girlsOnly}`);
    expect(offered).not.toContain(unknownSex);
  });

  it('narrows NOTHING for an unrestricted Level', async () => {
    // `any` means any: the filter is built from the Level's rule, not from a sex
    // the caller names, so an open Level offers everybody including a NULL sex.
    const offered = await ids(`eligible_for_level=${unrestricted}`);
    for (const id of [girl, boy, unknownSex]) expect(offered).toContain(id);
  });

  it('offers everybody when the parameter is absent', async () => {
    // Filtering by ROLE was rejected deliberately and stays rejected: minors
    // hold no role (§4.3) and a مؤطرة may legitimately be enrolled.
    const offered = await ids(`q=${encodeURIComponent(TAG)}`);
    for (const id of [girl, boy, unknownSex]) expect(offered).toContain(id);
  });

  it('narrows to nothing for an unknown Level, never back to everything', async () => {
    // A filter that silently stops filtering is worse than one returning empty.
    expect(await ids('eligible_for_level=00000000-0000-4000-8000-000000000000')).toEqual([]);
  });

  it('refuses a malformed Level id rather than ignoring it', async () => {
    const res = await call('GET', '/admin/users?eligible_for_level=nope', superAdmin);
    expect(res.status).toBe(400);
  });

  it('still publishes no `sex` on the contract', async () => {
    // The whole point of narrowing server-side: the eligible SET travels and the
    // fact behind it does not.
