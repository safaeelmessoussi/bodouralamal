...
beforeAll(async () => {
  ...
  subjectId = (await prisma.subject.create({ data: { name: `${TAG} مادة` } }))
    .id;
  untaughtSubjectId = (
    await prisma.subject.create({ data: { name: `${TAG} مادة غير مقررة` } })
  ).id;
  ...
  adminToken = bearer(await makeUser("مديرة فرع"), [
    { role: "admin", branches: null },
  ]);
  parentToken = bearer(await makeUser("والدة"), [
...
describe("POST /exams", () => {
  ...
  it("refuses `online` with a coded reason rather than a generic error", async () => {
    ...
    expect(
      await prisma.exam.count({
        where: { title: { startsWith: TAG }, mode: "online" },
      }),
    ).toBe(0);
  });
  ...
  it("refuses a subject the level does not teach, under the platform-wide name", async () => {
    const res = await call(
      ...
      "/exams",
      superAdmin,
      body({ subject_id: untaughtSubjectId }),
    );
  ...
  it("refuses a group that does not sit at this branch and level", async () => {
    ...
    expect(atOtherBranch.body.error?.details?.["reason"]).toBe(
      "BRANCH_MISMATCH",
    );
    ...
    expect(atOtherLevel.body.error?.details?.["reason"]).toBe(
      "SUBJECT_NOT_IN_LEVEL",
    );
  });

  it("refuses a parent and an anonymous caller", async () => {
    expect((await call("POST", "/exams", parentToken, body())).status).toBe(
      403,
    );
...
describe("DELETE /exams/{id}", () => {
  ...
  it("refuses a parent", async () => {
    const id = await createExam();
    expect((await call("DELETE", `/exams/${id}`, parentToken)).status).toBe(
      403,
    );
  });
});
...
describe("GET /exams", () => {
  it("filters by branch, level and date window", async () => {
    ...
    const elsewhere = await createExam({
      date: "2099-05-11",
      branch_id: branchB,
      room_id: roomB,
    });
    ...
    const byBranch = await call(
      "GET",
      `/exams?branch_id=${branchA}&page_size=100`,
      superAdmin,
    );
    ...
    const byLevel = await call(
      "GET",
      `/exams?level_id=${otherLevelId}&page_size=100`,
      superAdmin,
    );
...
describe("GET /calendar", () => {
  it("shows the sitting as its own kind, on its own date", async () => {
    ...
    const res = await call(
      "GET",
      "/calendar?from=2099-06-01&to=2099-06-30",
      superAdmin,
    );
    expect(res.status).toBe(200);

    const rows = (res.body["occurrences"] ?? res.body.data ?? []) as Record<
      string,
      unknown
    >[];
...
describe("an exam is restorable, and comes back whole (R59.3)", () => {
  it("reinstates the supervising staff removed with it", async () => {
    ...
    expect(
      (await call("POST", `/admin/trash/${entry!["id"]}/restore`, superAdmin))
        .status,
    ).toBe(200);
  ...
  it("refuses an Admin restoring or destroying it — the Trash is Super Admin only", async () => {
    ...
    expect(
      (await call("POST", `/admin/trash/${entry["id"]}/restore`, adminToken))
        .status,
    ).toBe(403);
    expect(
      (await call("DELETE", `/admin/trash/${entry["id"]}`, adminToken)).status,
    ).toBe(403);
