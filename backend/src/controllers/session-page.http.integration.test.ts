...
const content = { publicPdf: '', privateAudio: '', publicAudio: '', hiddenPdf: '' };
...
beforeAll(async () => {
  ...
  const room = await prisma.room.create({ data: { name: `${TAG} قاعة`, branchId: branchA } });
...
title: `${TAG} حلقة`,
  ...
  content.privateAudio = await makeContent('تسجيل-خاص', 'private', 'audio/mpeg');
...
describe('the page is public, at the caller’s tier (§5.2)', () => {
  ...
  it('returns the SAME occurrence shape the grid returns', async () => {
    ...
    expect(Object.keys(page.body.occurrence!).sort()).toEqual(Object.keys(fromGrid).sort());
  ...
  it('an enrolled student sees the private recording; a teacher also sees hidden', async () => {
    ...
    const asTeacher = await call(`/calendar/sessions/${sessionId}`, teacherToken);
...
describe('recordings and linked_content are disjoint', () => {
  ...
  it('each item carries exactly the four fields TD-3.4 names', async () => {
    ...
    for (const item of [...res.body.recordings!, ...res.body.linked_content!]) {
      ...
      for (const leak of ['storage_key', 'storage_bucket', 'mime_type', 'visibility']) {
...
describe('lookup failures', () => {
  it('a malformed id is 400 and an unknown one is 404', async () => {
    ...
    const missing = await call('/calendar/sessions/00000000-0000-4000-8000-000000000000');
    expect(missing.status).toBe(404);
    expect(missing.body.error?.code).toBe('NOT_FOUND');
  });

  it('never answers 401 — it is a public surface', async () => {
    const res = await call(`/calendar/sessions/${sessionId}`, 'not-a-real-token');
    expect(res.status).toBe(200);
  });
});
