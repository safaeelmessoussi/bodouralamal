...
async function idsOf(path: string): Promise<string[]> {
  ...
  return (res.body.data as unknown as Record<string, unknown>[]).map((r) => String(r['id']));
}

/** Only the rows this file created — other suites share the database. */
const mine = (rows: Record<string, unknown>[]): string[] =>
  rows.filter((r) => String(r['name']).startsWith(TAG)).map((r) => String(r['name']));
...
beforeAll(async () => {
  ...
  categoryA = (await prisma.category.create({ data: { name: `${TAG} ج فئة`, displayOrder: 1 } })).id;
  categoryB = (await prisma.category.create({ data: { name: `${TAG} أ فئة`, displayOrder: 2 } })).id;
  ...
  for (const [i, name] of [`${TAG} ج مادة`, `${TAG} أ مادة`, `${TAG} ب مادة`].entries()) {
    await prisma.subject.create({ data: { name, displayOrder: i + 1 } });
  }
  levelA1 = (
    await prisma.level.create({
      data: { name: `${TAG} ج مستوى`, categoryId: categoryA, genderRestriction: 'any' },
    })
  ).id;
  await prisma.level.create({
    data: { name: `${TAG} أ مستوى`, categoryId: categoryA, genderRestriction: 'any' },
  });
  levelB1 = (
    await prisma.level.create({
      data: { name: `${TAG} ب مستوى آخر`, categoryId: categoryB, genderRestriction: 'any' },
    })
  ).id;
  for (const [i, name] of [`${TAG} ج مجموعة`, `${TAG} أ مجموعة`, `${TAG} ب مجموعة`].entries()) {
    await prisma.administrativeGroup.create({
      data: { name, levelId: levelA1, branchId, displayOrder: i + 1 },
    });
  }
  await prisma.administrativeGroup.create({
    data: { name: `${TAG} مجموعة أخرى`, levelId: levelB1, branchId, displayOrder: 4 },
  });
});
...
describe.each([
  ...
  ['administrative-groups', '/admin/administrative-groups?page_size=100&', 'name'],
])('R76 sorting — %s', (_label, base, field) => {
  const q = (suffix: string): string => (base.endsWith('&') ? base + suffix : `${base}?${suffix}`);
  ...
  it('preserves BR-19 when nothing is asked, and sorts both ways when asked', async () => {
    ...
    const asc = await call('GET', q(`sort_by=${field}&sort_dir=asc`), superAdmin);
    const desc = await call('GET', q(`sort_by=${field}&sort_dir=desc`), superAdmin);
    expect(asc.status).toBe(200);
    expect(desc.status).toBe(200);
    const ascNames = mine(asc.body.data as unknown as Record<string, unknown>[]);
    expect(ascNames.length).toBeGreaterThan(1);
    expect(mine(desc.body.data as unknown as Record<string, unknown>[])).toEqual(
      [...ascNames].reverse(),
    );
    ...
    expect(ascNames).not.toEqual(mine(plain.body.data as unknown as Record<string, unknown>[]));
  });

  it('refuses an unknown field, a column name and a bad direction', async () => {
    for (const bad of ['nope', 'display_order', 'displayOrder', 'deleted_at', 'id']) {
      const res = await call('GET', q(`sort_by=${bad}`), superAdmin);
      expect(res.status, bad).toBe(400);
      expect(res.body.error?.code, bad).toBe('VALIDATION_FAILED');
    }
    expect((await call('GET', q(`sort_by=${field}&sort_dir=up`), superAdmin)).status).toBe(400);
    expect((await call('GET', q('sort_dir=desc'), superAdmin)).status).toBe(400);
  });
});
...
it('keeps each allow-list its own — `category` sorts Levels and nothing else', async () => {
  ...
  expect((await call('GET', '/admin/levels?sort_by=category', superAdmin)).status).toBe(200);
  expect((await call('GET', '/admin/categories?sort_by=category', superAdmin)).status).toBe(400);
  expect((await call('GET', '/admin/subjects?sort_by=category', superAdmin)).status).toBe(400);
});
...
describe('R76 reorder — flat collections (Categories, Subjects)', () => {
  it.each([
  ...
  ])('%s: writes contiguous 1..n from the sequence and persists it', async (_l, base) => {
    const before = await idsOf(base);
    const reversed = [...before].reverse();

    const res = await call('PATCH', `${base}/order`, superAdmin, { ids: reversed });
    ...
    expect((await call('PATCH', `${base}/order`, superAdmin, { ids: reversed })).status).toBe(200);
    expect(await idsOf(base)).toEqual(reversed);
  });
  ...
  it.each([
    ['categories', '/admin/categories'],
    ['subjects', '/admin/subjects'],
  ])('%s: refuses a duplicate, a foreign id and a partial sequence', async (_l, base) => {
    ...
    const partial = await call('PATCH', `${base}/order`, superAdmin, { ids: order.slice(0, 1) });
  ...
  it.each([
    ['categories', '/admin/categories'],
    ['subjects', '/admin/subjects'],
  ])('%s: inherits the write authority and is not captured by /:id', async (_l, base) => {
    const order = await idsOf(base);
    expect((await call('PATCH', `${base}/order`, teacher, { ids: order })).status).toBe(403);
    expect((await call('PATCH', `${base}/order`, undefined, { ids: order })).status).toBe(401);
    ...
    expect((await call('PATCH', `${base}/order`, superAdmin, { ids: order })).status).toBe(200);
  });
});
...
describe('R76 reorder — collections scoped to a parent (Levels, Groups)', () => {
  const cases = [
    ...
    },
    {
      ...
      list: () => `/admin/administrative-groups?level_id=${levelA1}&page_size=100`,
      within: () => levelA1,
      outsider: async (): Promise<string> =>
        (await idsOf(`/admin/administrative-groups?level_id=${levelB1}&page_size=100`))[0]!,
    },
  ];
  ...
  for (const c of cases) {
    ...
    it(`${c.label}: refuses a sequence that reaches outside the named parent`, async () => {
      ...
      const foreign = await (typeof c.outsider === 'function' ? c.outsider() : c.outsider);
      const res = await call('PATCH', `${c.base}/order`, superAdmin, {
        within: c.within(),
    ...
    it(`${c.label}: requires the parent — the sequence alone cannot name it`, async () => {
      const order = await idsOf(c.list());
      expect((await call('PATCH', `${c.base}/order`, superAdmin, { ids: order })).status).toBe(400);
    });

    it(`${c.label}: inherits the write authority`, async () => {
      const body = { within: c.within(), ids: await idsOf(c.list()) };
      expect((await call('PATCH', `${c.base}/order`, teacher, body)).status).toBe(403);
      expect((await call('PATCH', `${c.base}/order`, undefined, body)).status).toBe(401);
    });
  }
});
