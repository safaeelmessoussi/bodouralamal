...
async function content(
  label: string,
  over: { visibility?: string; branchId?: string | null; levelId?: string; forced?: boolean } = {},
): Promise<string> {
...
beforeAll(async () => {
  ...
  teacherToken = bearer(await makeUser('أستاذة'), [{ role: 'teacher', branches: [branchB] }]);
...
describe('§4.9 tiers filter every result set', () => {
  ...
  it('a Pending account sees exactly what an anonymous visitor sees (TD-1)', async () => {
    ...
    expect(new Set(await titlesFor(pendingToken))).toEqual(new Set(await titlesFor()));
  });
...
describe('the filter set is identical for everyone (§5.2)', () => {
  it('filters by level, subject, academic year and category', async () => {
    const byLevel = await call(`${MINE}${otherLevelId}&academic_year_id=${academicYearId}`);
    expect(byLevel.status).toBe(200);
    expect(byLevel.body.data!.every((r) => r.level_id === otherLevelId)).toBe(true);
    ...
    const bySubject = await call(`${scoped}${academicYearId}&subject_id=${subjectId}`);
    expect(bySubject.body.data!.every((r) => r.subject_id === subjectId)).toBe(true);
  });
