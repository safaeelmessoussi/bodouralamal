...
describe("coverage percent", () => {
  it("is a percentage of the Surah’s own total", () => {
    ...
    expect(computeCoverage([{ start: 1, end: 4 }], 7).coveragePercent).toBe(
      57.14,
    );
  });
  ...
  it("reaches exactly 100 for a complete Surah", () => {
    ...
    expect(computeCoverage([{ start: 1, end: 286 }], 286).coveragePercent).toBe(
      100,
    );
  });
  ...
  it("rounds to the two decimals the column stores", () => {
    ...
    expect(computeCoverage([{ start: 1, end: 1 }], 3).coveragePercent).toBe(
      33.33,
    );
  });
});
