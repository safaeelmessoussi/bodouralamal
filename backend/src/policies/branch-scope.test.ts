 ...
 */
const row = (role: string, branchId: string | null) => ({ branchId, role: { name: role } });
...
describe('toRoleScopes', () => {
  it('collapses several rows of one role into one entry', () => {
    const scopes = toRoleScopes([row('admin', MARRAKESH), row('admin', CASABLANCA)]);
    expect(scopes).toHaveLength(1);
    expect(scopes[0]!.role).toBe('admin');
    expect([...scopes[0]!.branches!].sort()).toEqual([CASABLANCA, MARRAKESH].sort());
  });
  ...
  it('a NULL branch means ALL branches, not "no scope"', () => {
    ...
    expect(toRoleScopes([row('admin', null)])).toEqual([{ role: 'admin', branches: null }]);
  });
  ...
  it('keeps roles separate rather than merging their branches', () => {
    const scopes = toRoleScopes([row('teacher', CASABLANCA), row('admin', MARRAKESH)]);
  ...
  it('rolesOf derives the role list', () => {
    const scopes = toRoleScopes([row('teacher', CASABLANCA), row('admin', null)]);
...
describe('canActOnBranch — per-role, never a union', () => {
  ...
  it('Super Admin bypasses branch restrictions by ROLE, not by a null scope', () => {
    ...
    const superAdmin: RoleScope[] = [{ role: 'super_admin', branches: [MARRAKESH] }];
...
describe('branchFilter', () => {
  it('is unrestricted for Super Admin', () => {
    expect(branchFilter([{ role: 'super_admin', branches: [MARRAKESH] }], ['admin'])).toEqual({});
  });

  it('is unrestricted for an all-branches assignment', () => {
    expect(branchFilter([{ role: 'admin', branches: null }], ['admin'])).toEqual({});
  });
  ...
  it('restricts to the named role\'s branches only', () => {
    ...
    expect(branchFilter(scopes, ['admin'])).toEqual({ id: { in: [MARRAKESH] } });
  });

  it('matches nothing when the role is not held', () => {
    expect(branchFilter([{ role: 'teacher', branches: [CASABLANCA] }], ['admin'])).toEqual({
      id: { in: [] },
    });
  });
});
