...
describe("weekly", () => {
  it("returns every matching weekday in the range", () => {
    ...
    expect(
      iso(expandSchedule(rule(), day("2026-06-01"), day("2026-06-30"))),
    ).toEqual([
      "2026-06-02",
      "2026-06-09",
      "2026-06-16",
  ...
  it("includes both endpoints of the range", () => {
    expect(
      iso(expandSchedule(rule(), day("2026-06-02"), day("2026-06-09"))),
    ).toEqual(["2026-06-02", "2026-06-09"]);
  });

  it("returns nothing when the range is inverted", () => {
    expect(
      expandSchedule(rule(), day("2026-06-30"), day("2026-06-01")),
    ).toEqual([]);
  });
});
...
describe("multiple_weekdays", () => {
  it("returns every listed weekday, in date order", () => {
    const r = rule({
      recurrence: "multiple_weekdays",
      weekdays: ["monday", "thursday"],
    });
    expect(
      iso(expandSchedule(r, day("2026-06-01"), day("2026-06-14"))),
    ).toEqual([
      "2026-06-01", // Mon
      "2026-06-04", // Thu
      "2026-06-08",
  ...
  it("ignores an unknown weekday name instead of matching it", () => {
    const r = rule({
      recurrence: "multiple_weekdays",
      weekdays: ["monday", "noneday"],
    });
    expect(
      iso(expandSchedule(r, day("2026-06-01"), day("2026-06-07"))),
    ).toEqual(["2026-06-01"]);
  });
});
...
describe("biweekly_alternating — the case §19.2 names explicitly", () => {
  ...
  it("shifting the anchor by one week selects the OPPOSITE weeks", () => {
    ...
    const onDates = iso(
      expandSchedule(on, day("2026-06-01"), day("2026-06-30")),
    );
    const offDates = iso(
      expandSchedule(off, day("2026-06-01"), day("2026-06-30")),
    );
...
describe("monthly and yearly", () => {
  it("monthly returns the same day-of-month each month", () => {
    ...
    expect(
      iso(expandSchedule(r, day("2026-06-01"), day("2026-08-31"))),
  ...
  it("monthly on the 31st simply skips months that have no 31st", () => {
    ...
    expect(
      iso(expandSchedule(r, day("2026-06-01"), day("2026-09-30"))),
  ...
  it("yearly returns one date per year", () => {
    ...
    expect(
      iso(expandSchedule(r, day("2026-01-01"), day("2028-12-31"))),
    ).toEqual(["2026-09-12", "2027-09-12", "2028-09-12"]);
  });
});
...
describe("daily", () => {
  it("returns every day in the range regardless of weekday set", () => {
    ...
    expect(
      expandSchedule(r, day("2026-06-01"), day("2026-06-07")),
    ).toHaveLength(7);
  });
});
...
describe("helpers", () => {
  it("mondayOf returns the Monday on or before any day, including Sunday", () => {
    expect(mondayOf(day("2026-06-01")).toISOString().slice(0, 10)).toBe(
      "2026-06-01",
    ); // Mon
    expect(mondayOf(day("2026-06-04")).toISOString().slice(0, 10)).toBe(
      "2026-06-01",
    ); // Thu
    ...
    expect(mondayOf(day("2026-06-07")).toISOString().slice(0, 10)).toBe(
      "2026-06-01",
    );
    expect(mondayOf(day("2026-06-08")).toISOString().slice(0, 10)).toBe(
      "2026-06-08",
    );
  });
...
describe("timesOverlap — half-open, so back-to-back classes are legal", () => {
  ...
  it("ignores the date part entirely — it compares clock times", () => {
    ...
    expect(
      timesOverlap(a, new Date(Date.UTC(2026, 5, 2, 10, 0, 0)), b, at(10, 30)),
    ).toBe(true);
  });
});
