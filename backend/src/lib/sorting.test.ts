...
describe("resolving a sort", () => {
  ...
  it("appends a deterministic tiebreaker to every order", () => {
    ...
    // or on neither.
    expect(
      resolveSort(FIELDS, { sortBy: "name", sortDir: "asc" }, FALLBACK),
  ...
  it("does not double the tiebreaker if the field already ends in id", () => {
    ...
    expect(resolveSort(ends, { sortBy: "x" }, FALLBACK)).toEqual([
      { x: "asc" },
      { id: "asc" },
    ]);
  });
  ...
  it("sorts ascending by default, and descending when asked", () => {
    expect(resolveSort(FIELDS, { sortBy: "name" }, FALLBACK)[0]).toEqual({
      name: "asc",
    });
    expect(
      resolveSort(FIELDS, { sortBy: "name", sortDir: "desc" }, FALLBACK)[0],
    ).toEqual({
      name: "desc",
    });
  });
  ...
  it("carries the direction into a relation traversal", () => {
    expect(
      resolveSort(FIELDS, { sortBy: "category", sortDir: "desc" }, FALLBACK),
    ).toEqual([
...
describe("an unknown sort is refused, never ignored", () => {
  it("refuses a field outside the allow-list, and says what is allowed", () => {
    ...
    expect(() =>
      resolveSort(FIELDS, { sortBy: "password" }, FALLBACK),
    ).toThrowError(/cannot sort by password/);
  });
  ...
  it("refuses a database column name that is not a contract field", () => {
    ...
    for (const injected of [
      ...
      expect(() =>
        resolveSort(FIELDS, { sortBy: injected }, FALLBACK),
      ).toThrowError(/cannot sort by/);
    }
  });

  it("refuses a direction that is neither asc nor desc", () => {
    expect(() =>
      resolveSort(FIELDS, { sortBy: "name", sortDir: "sideways" }, FALLBACK),
    ).toThrowError(/asc or desc/);
  });

  it("refuses a direction with no field — the caller believes they are sorting", () => {
    expect(() =>
      resolveSort(FIELDS, { sortDir: "desc" }, FALLBACK),
    ).toThrowError(/sort_dir requires sort_by/);
  });
});
