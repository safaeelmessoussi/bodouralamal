 ...
 */
const person = (
  over: Partial<{ publicDisplayName: string | null; nameArabic: string }> = {},
) => ({
...
describe("choosing the public name", () => {
  it("uses the chosen public name when set", () => {
    expect(
      publicDisplayName(person({ publicDisplayName: "أم عبد الله" })),
    ).toBe("أم عبد الله");
  });
  ...
  it("treats an empty or whitespace-only value as unset", () => {
    ...
    for (const blank of ["", "   ", "\t", "\n "]) {
      expect(publicDisplayName(person({ publicDisplayName: blank }))).toBe(
        "فاطمة الزهراء بنعلي",
      );
    }
  });
  ...
  it("never returns the legal name when a public one was chosen", () => {
    ...
    const result = publicDisplayName(
      person({ publicDisplayName: "أم عبد الله" }),
    );
