...
describe('flow state — the CSRF and PKCE material', () => {
  ...
  it('derives the PKCE challenge as S256 of the verifier', () => {
    ...
    expect(codeChallengeFor(codeVerifier)).not.toBe(codeChallengeFor(createFlowState().codeVerifier));
...
describe('sealing the flow state (Revision 16 F5)', () => {
  ...
  it('rejects a tampered PAYLOAD', () => {
    ...
    const forged = Buffer.from(
      JSON.stringify({ state: 'attacker-chosen', codeVerifier: flow.codeVerifier }),
    ).toString('base64url');
  ...
  it('rejects a seal made with a different signing key', () => {
    const sealed = sealFlowState(createFlowState(), 'a-completely-different-signing-key-value');
  ...
  it('does NOT accept a payload signed with the raw JWT key', () => {
    ...
    const rawSig = createHmac('sha256', KEY).update(payload).digest('base64url');
  ...
  it('rejects a correctly-signed payload that is not a flow state', () => {
    ...
    const key = createHmac('sha256', KEY).update('oauth-flow-state:v1').digest('base64url');
...
describe('exchangeCode — what Google says is not taken on trust', () => {
  ...
  it('accepts a verified email for OUR client and lowercases it', async () => {
    ...
    expect(result).toEqual({
      ok: true,
      identity: { email: 'safae.el@example.com', providerSubjectId: 'google-sub-1' },
    });
  });
  ...
  it('refuses an unverified email as its own distinct reason (§4.1b step 7)', async () => {
    ...
    const absent = await exchangeCode({
      ...base,
      fetchImpl: respondWith({
        id_token: idToken({ aud: 'client-123', sub: 'google-sub-1', email: 'a@example.com' }),
      }),
    });
  ...
  it('collapses every upstream failure into oauth_unavailable, leaking nothing', async () => {
    ...
    const nonOk = await exchangeCode({ ...base, fetchImpl: respondWith({}, false) });
    expect(nonOk).toEqual({ ok: false, reason: 'oauth_unavailable' });

    const noIdToken = await exchangeCode({ ...base, fetchImpl: respondWith({ access_token: 'x' }) });
  ...
  it('sends the verifier and the code, not the challenge', async () => {
    ...
    expect(new URLSearchParams(sentBody).get('grant_type')).toBe('authorization_code');
  });
});
