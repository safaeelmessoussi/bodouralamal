...
describe('signed PUT + signed GET round-trip through the Nginx /storage proxy (§3.1, §18)', () => {
  it('round-trips a private-bucket object without SignatureDoesNotMatch', async () => {
    ...
    expect(putRes.ok, `presigned PUT through the proxy failed: ${putRes.status} ${putBody}`).toBe(
      true,
    );

    const getUrl = await presignGetUrl(clients, BUCKETS.private, key);
    const getRes = await fetch(getUrl);
    expect(getRes.ok, `presigned GET through the proxy failed: ${getRes.status}`).toBe(true);
  ...
  it('serves the private bucket ONLY through a signature — never a stable URL (§20 rule 4)', async () => {
    ...
    const putUrl = await presignPutUrl(clients, BUCKETS.private, key);
    expect((await fetch(putUrl, { method: 'PUT', body: payload })).ok).toBe(true);
    ...
    const unsigned = await fetch(`${config.STORAGE_BASE_URL}/${BUCKETS.private}/${key}`, {
      redirect: 'manual',
    });
  ...
  it('maps a stale/missing public link to /content-unavailable, never raw S3 XML (§3.1)', async () => {
    const res = await fetch(`${config.STORAGE_BASE_URL}/${BUCKETS.public}/does-not-exist.bin`, {
      redirect: 'manual',
    });
    ...
    expect(new URL(location, config.STORAGE_BASE_URL).pathname).toBe('/content-unavailable');
  ...
  it('answers a rejected upload with a real error, not the friendly-page redirect', async () => {
    ...
    const badSignature = `${config.STORAGE_BASE_URL}/${BUCKETS.private}/regression.bin`
      + '?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Signature=deadbeef';

    const res = await fetch(badSignature, { method: 'PUT', body: 'x', redirect: 'manual' });
  ...
  it('rejects an expired signature (TD-12 short-lived URLs)', async () => {
    ...
    const putUrl = await presignPutUrl(clients, BUCKETS.private, key);
    expect((await fetch(putUrl, { method: 'PUT', body: randomBytes(64) })).ok).toBe(true);
