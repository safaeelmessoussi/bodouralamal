...
import { issueUploadTicket, UPLOAD_TICKET_TTL_SECONDS, verifyUploadTicket } from './upload-token.js';
...
describe('the upload ticket', () => {
  ...
  it('refuses a payload edited to widen the scope', () => {
    ...
    const tampered = Buffer.from(JSON.stringify({ ...claims, branch_id: null, exp: 2 ** 31 }))
      .toString('base64url');
  ...
  it('refuses a malformed token rather than throwing', () => {
    ...
    expect(verifyUploadTicket('not-a-token', KEY)).toEqual({ valid: false, reason: 'malformed' });
