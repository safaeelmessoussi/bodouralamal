...
describe("access token (TD-12)", () => {
  ...
  it("rejects a tampered payload — privilege escalation must not verify", () => {
    ...
    const tamperedPayload = Buffer.from(JSON.stringify(forged)).toString(
      "base64url",
    );
  ...
  it("rejects an expired token", () => {
    const issuedLongAgo = new Date(
      Date.now() - (ACCESS_TTL_SECONDS + 60) * 1000,
    );
