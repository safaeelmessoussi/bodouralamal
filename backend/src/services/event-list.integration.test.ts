...
const scopedAdmin = (branchId: string): Actor =>
  actorOf([{ role: "admin", branches: [branchId] }]);
...
async function makeEvent(
  ...
  const event = await prisma.event.create({
    data: {
      ...
      ...(opts.recurrenceEnd
        ? { recurrenceEndDate: new Date(opts.recurrenceEnd) }
        : {}),
    },
  });
...
describe("branch narrowing (TD-2)", () => {
  ...
  it("refuses a caller who is neither admin nor teacher", async () => {
    ...
    await expect(listEvents(prisma, parent, {})).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
  });
});
