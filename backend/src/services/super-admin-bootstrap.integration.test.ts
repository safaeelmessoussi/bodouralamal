...
// R80.2 — the seed refuses to CREATE an account without one, by design. Set
// here rather than defaulted in the seed, which is the whole point: a default
  ...
  const users = await prisma.user.findMany({
    where: {
      OR: [
        ...
        { preProvisionedEmail: { contains: "sa-boot-" } },
      ],
    },
    select: { id: true },
  });
  ...
  if (parkedIds.length > 0) {
    ...
    for (const id of parkedIds) {
      if (!restored.includes(id)) {
        throw new Error(
          `FAILED TO RESTORE parked Super Admin ${id} — it is still not active`,
        );
      }
    }
  }
...
describe("§15.1 Revision 22 — SUPER_ADMIN_EMAIL is a bootstrap value", () => {
  ...
  it("lowercases the address, so a capitalised .env value stays claimable", async () => {
    ...
    expect(
      await prisma.user.count({
        where: { preProvisionedEmail: "sa-boot-caps@example.com" },
      }),
    ).toBe(1);
  });
  ...
  it("is a NO-OP once an active Super Admin exists — even under a different email", async () => {
    ...
    expect(
      await prisma.user.count({
        where: { preProvisionedEmail: "sa-boot-changed@example.com" },
      }),
    ).toBe(0);
  });
  ...
  it("is idempotent: re-running with the SAME email adds nothing", async () => {
    ...
    expect(
      await prisma.userBranchRole.count({
        where: {
          roleId: await superAdminRoleId(),
          user: { preProvisionedEmail: "sa-boot-same@example.com" },
        },
      }),
    ).toBe(1);
  });
  ...
  it("GRANTS the role to an existing account for that address rather than duplicating it", async () => {
    ...
    expect(
      await prisma.user.count({
        where: { preProvisionedEmail: "sa-boot-member@example.com" },
      }),
    ).toBe(0);
  });
  ...
  it("ACTIVATES a matched non-active account, so bootstrap yields a usable admin", async () => {
    ...
    expect(
      (await prisma.user.findUnique({ where: { id: suspended.id } }))
        ?.accountStatus,
    ).toBe("active");
  ...
  it("REFUSES to resurrect a soft-deleted holder, and creates nothing", async () => {
    ...
    await expect(
      bootstrapSuperAdmin(prisma, "sa-boot-deleted@example.com"),
    ).rejects.toThrow(/soft-deleted account/);
    ...
    expect(
      (await prisma.user.findUnique({ where: { id: deleted.id } }))?.deletedAt,
    ).toBeInstanceOf(Date);
    expect(
      await prisma.user.count({
        where: {
  ...
  it("a soft-deleted Super Admin does not hold the gate open", async () => {
    ...
    const first = onlyId(
      await activeSuperAdmins(),
      "soft-deleted Super Admin test",
    );
    await prisma.user.update({
      where: { id: first },
...
describe("§15.1 Revision 23 — SUPER_ADMIN_EMAIL is conditionally required", () => {
  it("fails LOUDLY, naming the variable, when the gate is open and it is absent", async () => {
    ...
    await expect(bootstrapSuperAdmin(prisma, undefined)).rejects.toThrow(
      /SUPER_ADMIN_EMAIL/,
    );
    await expect(bootstrapSuperAdmin(prisma, "   ")).rejects.toThrow(
      /SUPER_ADMIN_EMAIL/,
    );
    expect(await activeSuperAdmins()).toHaveLength(0);
  });
  ...
  it("is NOT required once an active Super Administrator exists", async () => {
    ...
    await expect(
      bootstrapSuperAdmin(prisma, undefined),
