...
beforeEach(async () => {
  ...
  for (const parentId of [parentAId, parentBId]) {
    await prisma.familyLink.create({ data: { parentId, studentId, status: 'approved' } });
  }
});
...
describe('the item appears, and is about the ARRANGEMENT', () => {
  it('groups every stamped link of one student into ONE item', async () => {
    ...
    expect(item.applicants.map((a) => a.role)).toEqual(['applicant', 'parent', 'parent']);
  });
...
describe('an administrator decides', () => {
  it('APPROVE means the links stand — the stamp clears, nothing else moves', async () => {
    await raise();
    const result = await decide(prisma, await actorFor(prisma, adminId), studentId, {
      approve: true,
    });
    ...
    expect(
      (await listApprovals(prisma, await actorFor(prisma, adminId), { type: 'identity-review' }))
        .data,
    ).toHaveLength(0);
  });
  ...
  it('records WHAT WAS DECIDED and, on revocation, what it did', async () => {
    ...
    expect((decided?.detail as Record<string, unknown>)['outcome']).toBe('links_revoked');
  ...
  it('refuses a rejection with no reason (§5.6)', async () => {
    await raise();
    await expect(
      decide(prisma, await actorFor(prisma, adminId), studentId, { approve: false }),
