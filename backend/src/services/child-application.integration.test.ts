  ...
  const users = await prisma.user.findMany({
    where: {
      OR: [
        ...
        { firstNameArabic: { startsWith: TAG } },
      ],
    },
    select: { id: true },
  });
      ...
      children: [
        {
          ...
          sex: "male",
          // R130 — every child application states her/his own date of birth.
          birthDate: new Date("2015-03-10T00:00:00Z"),
          consentMediaRelease: true,
        },
        {
          firstNameArabic: `${TAG} سارة`,
          lastNameArabic: "العلوي",
          sex: "female",
          birthDate: new Date("2017-08-22T00:00:00Z"),
...
describe("one request, many children", () => {
          ...
          children: [
            {
              ...
              sex: "female" as const,
              birthDate: new Date("2016-01-01T00:00:00Z"),
              consentMediaRelease: false,
            },
          ],
        }),
  ...
  it("captures media-release PER CHILD", async () => {
    ...
    expect(new Set(rows.map((r) => r.consentMediaRelease))).toEqual(
      new Set([true, false]),
    );
  });
});
...
describe("consent is materialised with the SUBMISSION values (R62.3b)", () => {
  it("records the text version the parent actually saw, not the current one", async () => {
    ...
    const { childUserId } = await decideChildApplication(
      prisma,
      admin,
      applicationIds[0]!,
      {
...
describe("R64 — the rules the two registration paths must share", () => {
        ...
        children: [
          {
            ...
            sex: "female",
            birthDate: new Date("2014-06-15T00:00:00Z"),
            consentMediaRelease: true,
          },
        ],
      }),
  ...
  it("records the branch asked for, per child", async () => {
        ...
        children: [
          {
            ...
            lastNameArabic: "العلوي",
            sex: "female",
            birthDate: new Date("2013-11-02T00:00:00Z"),
    ...
    await prisma.childApplication.deleteMany({
      where: { requestedBranchId: branch.id },
    });
...
describe("the parent role appears once, on the first approval (R62.9)", () => {
  it("grants it on the first child and not again on the second", async () => {
    ...
    const first = await decideChildApplication(
      prisma,
      admin,
      applicationIds[0]!,
      {
    ...
    const second = await decideChildApplication(
      prisma,
      admin,
      applicationIds[1]!,
      {
...
describe("the approved child (R62.5)", () => {
  it("receives a reference code, and it is unique", async () => {
    ...
    const a = await decideChildApplication(prisma, admin, applicationIds[0]!, {
      approve: true,
      ...place(),
    });
    const b = await decideChildApplication(prisma, admin, applicationIds[1]!, {
      approve: true,
      ...place(),
    });
    ...
    const [ca, cb] = await Promise.all([
      prisma.user.findUnique({
        where: { id: a.childUserId! },
        select: { referenceCode: true },
      }),
      prisma.user.findUnique({
        where: { id: b.childUserId! },
        select: { referenceCode: true },
      }),
    ]);
  ...
  it("is created with no login of its own (§4.3)", async () => {
    ...
    expect(
      await prisma.userIdentity.count({ where: { userId: childUserId! } }),
    ).toBe(0);
  });
});
...
describe("duplicate matching is proposed, never automatic (R62.3)", () => {
  ...
  it("links the chosen account instead of creating a duplicate", async () => {
    ...
    expect(
      await prisma.familyLink.count({ where: { studentId: existing.id } }),
    ).toBe(1);
...
describe("a submitted request reaches the approval queue", () => {
  ...
  it("carries each child's OWN application id, which is what decide acts on", async () => {
    ...
    expect(item.children.map((c) => c.applicationId).sort()).toEqual(
      [...applicationIds].sort(),
    );
  });
  ...
  it("drops a decided child and keeps its pending sibling", async () => {
    ...
    const page = await listApprovals(prisma, admin, {
      type: "child-application",
    });
    const item = page.data.find((i) => i.id === requestId);
  ...
  it("does not disturb the other queue types", async () => {
    ...
    const registrations = await listApprovals(prisma, admin, {
      type: "registration",
    });
