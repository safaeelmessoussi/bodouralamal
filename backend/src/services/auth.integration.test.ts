...
async function makeUser(opts: {
  ...
  const user = await prisma.user.create({
    data: {
      ...
      ...(opts.preProvisionedEmail ? { preProvisionedEmail: opts.preProvisionedEmail } : {}),
...
async function clear(): Promise<void> {
  await prisma.auditLog.deleteMany({ where: { actor: { nameArabic: { startsWith: TAG } } } });
  await prisma.userIdentity.deleteMany({ where: { user: { nameArabic: { startsWith: TAG } } } });
...
describe('§4.1b login resolution and routing', () => {
  it('4c — an unknown person is routed to onboarding, and nothing is persisted', async () => {
    ...
    const route = await resolveLogin(prisma, { email, providerSubjectId: 'sub-unknown' });
    ...
    expect(await prisma.userIdentity.count({ where: { providerSubjectId: 'sub-unknown' } })).toBe(0);
  ...
  it('4b — a pre-provisioned account binds on first login and writes auth.identity_bound', async () => {
    const email = uniqueEmail();
    const userId = await makeUser({ status: 'active', preProvisionedEmail: email });

    const route = await resolveLogin(prisma, { email, providerSubjectId: 'sub-bind' });
  ...
  it('4a — the second login resolves by identity and does NOT re-bind', async () => {
    ...
    const second = await resolveLogin(prisma, { email, providerSubjectId: 'sub-twice' });
    ...
    expect(
      await prisma.auditLog.count({ where: { targetId: userId, actionType: 'auth.identity_bound' } }),
    ).toBe(1);
  });
  ...
  it('Pending routes to the status screen, never to a dashboard (TD-1)', async () => {
    ...
    const route = await resolveLogin(prisma, { email, providerSubjectId: 'sub-pending' });
  ...
  it.each([
  ...
  ])('%s routes to the deactivated screen and is NEVER bound (R16)', async (status, reason) => {
    const email = uniqueEmail();
    const userId = await makeUser({ status, preProvisionedEmail: email });

    const route = await resolveLogin(prisma, { email, providerSubjectId: `sub-${status}` });
    ...
    expect((denied!.detail as Record<string, unknown>)['reason']).toBe(reason);
  });
  ...
  it('a BOUND account with deleted_at set is deactivated though its status is active (F6)', async () => {
    ...
    const route = await resolveLogin(prisma, { email, providerSubjectId: 'sub-deleted' });
    expect(route.kind).toBe('deactivated');
  ...
  it('a deactivated identity (user soft-deleted after binding) does not resolve (TD-5)', async () => {
    ...
    await prisma.userIdentity.updateMany({ where: { userId }, data: { isActive: false } });
    await prisma.user.update({ where: { id: userId }, data: { deletedAt: new Date() } });

    const route = await resolveLogin(prisma, { email, providerSubjectId: 'sub-deact' });
    expect(route.kind).toBe('deactivated');
  });
