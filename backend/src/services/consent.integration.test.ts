 ...
 * surfaced.
 */
async function makeGroupInOwnLevel(
  branchId: string,
  name: string,
): Promise<string> {
...
describe("§4.1a — recording a decision declared in person", () => {
  it("records a grant with method staff_recorded and the active text version", async () => {
    ...
    const result = await recordStaffConsent(
      prisma,
  ...
  it("a revocation records who revoked it and when", async () => {
    const { admin, student } = await scenario();
    const { recordId } = await recordStaffConsent(
      prisma,
      await actorFor(prisma, admin),
      student,
      {
...
describe("BR-1 — effective status", () => {
  ...
  it("the MOST RECENT record wins, not the first", async () => {
    ...
    expect(
      (await effectiveConsent(prisma, student))["media_release"]?.granted,
    ).toBe(false);
  });
...
describe("§4.1a + TD-4 — the re-evaluation enqueue", () => {
  it("enqueues consent.reevaluate for the student's group", async () => {
    ...
    expect(result.reevaluatedSessions).toEqual([
      contexts.get(groupId)!.sessionId,
    ]);
...
describe("TD-2 / TD-12 — who may record", () => {
  ...
  it("readConsent enforces the same audience", async () => {
    ...
    await expect(
      readConsent(prisma, await actorFor(prisma, admin), student),
    ).resolves.toBeTruthy();
    await expect(
      readConsent(prisma, await actorFor(prisma, teacher), student),
  ...
  it("a revocation writes consent.revoke", async () => {
    ...
    expect(
      await prisma.auditLog.count({
        where: { actorUserId: admin, actionType: "consent.revoke" },
      }),
    ).toBe(1);
  });
});
