...
import { eventsStaffedBy, isResponsibleForEvent } from '../policies/roster-resolution.js';
...
describe('R71 — a مؤطرة responsible for an event may manage it', () => {
  it('with NO teaching assignment at all', async () => {
    ...
    const before = await failure(() =>
      updateEvent(prisma, teacher(her), eventId, 0, { title: `${TAG} حفل معدّل` }),
    );
    ...
    await updateEvent(prisma, teacher(her), eventId, 0, { title: `${TAG} حفل معدّل` });
  ...
  it('an ASSISTANT may not edit it', async () => {
    ...
    const denied = await failure(() =>
      updateEvent(prisma, teacher(helper), eventId, 0, { title: `${TAG} محاولة` }),
    );
    expect(denied.code).toBe('NOT_FOUND');
    // …but she IS on the event, which is what makes it visible to her.
    expect((await eventsStaffedBy(prisma, helper)).get(eventId)).toBe('assistant');
  ...
  it('an unrelated مؤطرة reaches nothing', async () => {
    ...
    const denied = await failure(() =>
      updateEvent(prisma, teacher(stranger), eventId, 0, { title: `${TAG} تسلل` }),
    );
    expect(denied.code).toBe('NOT_FOUND');
...
describe('R71 — creating an event is what makes a مؤطرة answerable', () => {
  it('records the creating مؤطرة as responsible, in the same transaction', async () => {
    ...
    await prisma.recurringCourseSchedule.deleteMany({ where: { id: schedule.id } });
...
describe('R71 — reconciliation is tombstone-and-revive (R59)', () => {
  it('tombstones a removed assistant and REVIVES her on return', async () => {
    ...
    const first = await prisma.eventStaff.findFirstOrThrow({ where: { eventId, userId: helper } });

    await setEventStaff(prisma, superAdmin(), eventId, []);
    const tombstoned = await prisma.eventStaff.findFirstOrThrow({ where: { id: first.id } });
    ...
    const revived = await prisma.eventStaff.findFirstOrThrow({ where: { id: first.id } });
    // The SAME row — not a second one beside a tombstone.
    expect(revived.deletedAt).toBeNull();
    expect(revived.position).toBe('responsible');
    expect(await prisma.eventStaff.count({ where: { eventId, userId: helper } })).toBe(1);
  });
  ...
  it('earns no Trash entry — it is one field of an update (R59)', async () => {
    ...
    expect(await prisma.trash.count({ where: { targetEntity: 'EventStaff' } })).toBe(0);
  });
  ...
  it('writes its own audit action, distinct from event.update', async () => {
    ...
    expect(
      await prisma.auditLog.count({ where: { targetId: eventId, actionType: 'event.staff_change' } }),
    ).toBe(1);
  });
});
...
describe('R71 — educational authorization is unchanged', () => {
  it('a مؤطرة still reaches an event through her TEACHING scope alone', async () => {
    ...
    const subject = await prisma.subject.create({ data: { name: `${TAG} مادة تدريس` } });
    const schedule = await prisma.recurringCourseSchedule.create({
      data: {
        title: `${TAG} حصة تدريس`,
    ...
    await updateEvent(prisma, teacher(her), eventId, 0, { title: `${TAG} نشاط معدّل` });
  ...
  it('a Super Admin is unaffected in either direction', async () => {
    ...
    await updateEvent(prisma, superAdmin(), eventId, 0, { title: `${TAG} نشاط مشرفة` });
    expect((await prisma.event.findUnique({ where: { id: eventId } }))?.title).toBe(
      `${TAG} نشاط مشرفة`,
    );
  });
});
