...
import { FramingPreferenceSummary, FramingPreferenceValue } from './framing-preference-summary.js';

describe('general framing preference summary', () => {
  it('shows online without inventing a physical branch', () => {
    const html = renderToStaticMarkup(
      <FramingPreferenceValue
        framing={{ mode: 'online', all_branches: false, branches: [] }}
      />,
    );
  ...
  it('keeps legacy profiles honest and labels the preference as read-only planning data', () => {
    const html = renderToStaticMarkup(<FramingPreferenceSummary framing={null} />);
    expect(html).toContain(ar.framing.notStated);
    expect(html).toContain(ar.framing.generalTitle);
    expect(html).toContain(ar.framing.generalHint);
  });
});
