...
- R62 is drafted at SRS Revision 62 ([ledger](../archive/SRS-revisions-R1-R173.md)), audited against the live architecture in its §62.13, not applied. The audit changed the draft once: deferring the child's `User` to approval broke consent (`ConsentRecord.student_id` cannot precede the student), so §62.3b captures consent on the application at submission with the text version in force and materialises it at approval.
