...
3. Use an empty target Compose project and a nonexistent private recovered-config directory.
   ...
   `docker-compose.yml` already defines the one SeaweedFS model every tier shares (Owner
   decision, 2026-09-20), so there is no separate storage file to pass here any more:
