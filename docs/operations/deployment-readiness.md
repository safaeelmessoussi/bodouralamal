...
| **OWNER INPUT REQUIRED — ONLINE-CLASS MEDIA PROVIDER** (found 2026-09-20, SRS R163 §4) | No release tier has a media provider. Staging carries no `LIVEKIT_*` setting and no media server, so «دخول الحصة» truthfully answers *«خدمة الحصص عن بُعد غير مهيّأة بعد»* and recording answers the same. Localhost alone has one (the dev overlay's LiveKit, Redis and Egress) — every R98/R99 verification ran there | Not provisioned, because the two routes differ in ways engineering must not choose between. **LiveKit Cloud:** the Owner's account and credentials, a paid tier beyond roughly two classes a month, beneficiaries' live voice and video routed outside Morocco (a Law 09-08 question for real data), and no recording into this deployment's object store — its Egress needs a public `https://` S3 endpoint. **Self-hosted on the VPS:** media stays in-country, but it needs UDP/TCP media ports published to the internet, which reverses §19.1's *only Nginx publishes ports, and only 80/443* (enforced by host preflight and CI), and memory a 3.7-GiB Staging host does not obviously have once Egress's headless browser is counted. See [the provider decision](../development/online-class-provider.md). Production needs the same answer before remote classes can be offered |
