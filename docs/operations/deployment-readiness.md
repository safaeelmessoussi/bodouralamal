...
|---|---|---|
| **B1 — ONE MODEL IN THE REPOSITORY; RUNNING ON LOCALHOST; STAGING AWAITS ITS DEPLOYMENT; PRODUCTION ROLLOUT SEPARATELY UNAUTHORIZED** | The final MinIO OSS release is affected and unsupported | Pinned SeaweedFS 4.46 is the only object store the repository defines, for Localhost, Staging and Production alike (Owner decision, 2026-09-20 — Localhost/Staging object data is discarded and recreated, not migrated). The Localhost stack was switched on 2026-09-20 (its `minio` service recreated on the new `bodour_seaweedfs-data` volume; the old `bodour_minio-data` volume is retained, unreferenced, until deleted by hand). **Staging still runs its pre-change MinIO container until the next authorized deployment.** That deployment needs no data migration, but host preflight refuses to run while the legacy `bodour_minio-data` volume exists, so an operator must first remove it explicitly (stop the old `minio` container, then `docker volume rm bodour_minio-data` — Staging object data is already Owner-authorized to be discarded); the pipeline then creates the SeaweedFS volume empty. It has separate physical storage, repeatable explicit initialization, bucket-policy/integrity/edge tests and same-version restart/recreate/recovery proof. See [B1 evidence](../development/testing.md#b1-seaweedfs-compatibility-and-recovery). Production will run the identical implementation, but that decision only fixed which implementation — Production's own go-live remains a separate, still-open decision, and the legacy MinIO pin remains prohibited for it either way |
