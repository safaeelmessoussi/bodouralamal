...
| **IMPLEMENTED — HOSTED VERIFICATION REQUIRED** | GitHub warned that the v4 checkout/setup-node actions targeted deprecated Node 20 | Document Owner approval was received on 2026-08-30; all invocations use the maintained v7 Node 24 lines, preserve the explicit Node/cache inputs, and are pinned by the CI portability guard. Close only after the complete hosted run and exact-image publication are terminal and green |
