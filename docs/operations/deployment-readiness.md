...
| Recovery | Encrypted **same-VPS** recovery-point tooling ([B8, Owner decision 2026-09-12](recovery.md)) preflights the repository before outage, refuses while a class is being recorded (R167 §5 — the same `ops:active-recordings` gate a deployment asks), preserves exact container identities, keeps the R133 monthly/max-two retention, runs from a daily systemd timer, and passes raw-volume plus clean logical-PostgreSQL restore. **No remote or offsite target exists or is claimed**; the same-VPS arrangement protects against logical failure only. Still open: the realistic host drill on the selected VPS and the backup-failure alarm on the Admin surface (below) |
...
| **DECIDED — SRS Revision 170 §18** | Audit identity email, exact content-coordinate wording, and required free-text evidence conflicted with the no-redundant-PII boundary | The Owner decided (2026-09-22): user id only, a content id plus a non-reversible coordinate digest, fixed codes in the audit with the sentence only on the record that owns it. Implemented and pinned by tests; no open question remains |
| **PARTLY IMPLEMENTED — SRS Revision 169 §11** | TD-14/TD-16 require terminal-job, queue-lag, backup and TLS alarms on the Admin dashboard | `GET /admin/operations/status` («حالة النظام», Super Admin only) now answers failed and late background jobs and unfinished storage retirements, in counts. **Backup freshness and certificate expiry are still host-only** (`scripts/backup/check-readiness.sh`, the certbot timer): they never reach the Admin surface, because the API cannot read the host's restic status file or the certificate volume. Closing that half is a bounded decision for the Owner — publish those two host facts to the API (a read-only file the API mounts) or leave them runbook-visible |
...
| **PARTIAL — INTEGRATION COMPLETE** | Permission/E2E/coverage gates in hosted CI | Full real-stack integration and all-table isolation now gate release. Three §19.2 gates are still absent (Codex review, 2026-09-22): a **generated permission-matrix** test (today each route's authority is pinned by its own HTTP tests, not by one table derived from TD-3), **J1–J8 as automation** (today: real-browser harnesses under `scripts/dev/browser/` against Localhost, and the anonymous 15/15 Staging smoke), and the **≥80 % coverage threshold** (coverage is collected, not enforced). Each is bounded work; none needs a Production host |
| **DONE — §3.1a PHASE 2, PART** | Container image digests | Every third-party image in `docker-compose.yml` and both Dockerfiles is pinned to the index digest Staging verified (2026-09-22); `check-compose-operations.sh` refuses an unpinned image. See [environments](environments.md#version-and-image-pinning) |
| **RECORDED — DEPENDENCY AUDIT** | `npm audit` at the feature freeze | Patch-level fixes applied in their own commit (`qs`, `fast-uri`, `brace-expansion`, `nanoid`, `postcss`, `vitest`, 2026-09-22). What remains in the backend's production set is **one chain under the `prisma` CLI** (`@prisma/dev` → `find-my-way`, `valibot`, `mysql2`; `@prisma/config` → `deepmerge-ts`): every 7.x is in the advisory range and the only fix is a major upgrade, prohibited by §3.1a without an Owner-approved task. **Accepted for now with the reason recorded:** none of those modules runs in this platform — no MySQL driver is opened, the Prisma local dev server never starts, and the CLI's only Production act is `migrate deploy` against a config the operator controls. Re-run the audit in the dedicated upgrade task |
