...
| Storage | Three internal buckets: public, private, recording staging. Production Compose selects the [B1 SeaweedFS replacement](../architecture/storage.md#b1-candidate-verification-checkpoint); authenticated S3 readiness and exact-coordinate Nginx authorization remain. Existing Local/Staging MinIO is not migrated |
...
|---|---|---|
| **B1 LOCALLY VERIFIED — LIVE ROLLOUT NOT AUTHORIZED** | The final MinIO OSS release is affected and unsupported | Owner-authorized engineering selection is pinned SeaweedFS 4.46, with separate physical storage, repeatable explicit initialization, bucket-policy/integrity/edge tests and same-version restart/recreate/recovery proof. See [B1 evidence](../development/testing.md#b1-seaweedfs-compatibility-and-recovery). The legacy MinIO pin remains prohibited for Production; real provisioning, any populated migration and residency evidence remain operational prerequisites |
