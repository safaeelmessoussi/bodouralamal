...
| `GET` | `/admin/approvals` | 🔒 | Registration rows carry `role_requests[]` (`kind`, `status`, `first_time`) and `circle_preferences[]` (R168 §1), and an ACTIVE account with a request still pending stays listed. Deliberately **unscoped**. Registration rows provide an authorized complete-details projection. `review_user_id` returns at most that applicant's still-pending review; stale coordinates return an empty page. Compact Branch/Category is shown only when unambiguous; exact values remain per child. |
