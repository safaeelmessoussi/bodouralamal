...
| `GET` | `/profile/role-requests` | 🔒 | **R169 §1.** The caller's own role requests (`kind`, `status`, `decided_at`) and `askable` — the kinds she may ask for now: not held, not waiting, never `guardian` (registering a child is that request). The decline reason is operator-facing and is never in this projection |
