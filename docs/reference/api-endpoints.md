...
| | Path | Audience | Notes |
|---|---|---|---|
| `POST` | `/registrations` | 🌐 + token | Gated by the signed onboarding token; identity comes solely from it. New applicants require phone. Adult requests carry their Branch/Category; every child carries her own Branch/Category and submitted details. A `requested_role: 'teacher'` request instead requires R115 framing. **R168 §1 — `kind: 'roles'`** asks for any combination of `student · guardian · teaching · administration` with ONE `applicant` and a section per role asked for (`student.first_time`, and for a first registration her ranked `circle_preferences`); no field names an administrative role. Requests grant nothing. |
| `GET` | `/admin/approvals` | 🔒 | Registration rows carry `role_requests[]` (`kind`, `status`, `first_time`) and `circle_preferences[]` (R168 §1), and an ACTIVE account with a request still pending stays listed — marked `account_active` (R169 §1), which means the item is decided per role and never whole. Deliberately **unscoped**. Registration rows provide an authorized complete-details projection. `review_user_id` returns at most that applicant's still-pending review; stale coordinates return an empty page. Compact Branch/Category is shown only when unambiguous; exact values remain per child. |
...
| `POST` | `/admin/approvals/{id}/reject` | 🔒 | Body carries a reason. Both whole-account acts answer `DECIDE_PER_ROLE` for a registration with several role requests or a lone `administration` one (R168 §1) |
| `POST` | `/admin/approvals/{id}/roles/{kind}/approve` | 🔒 | **One requested role, decided on its own** (R168 §1); `{id}` is the applicant, `{kind}` is `student · guardian · teaching · administration`. `.strict()` body: `student` → `{ enrollment }` (R66.5: a group, or a Level and its branch); `teaching` / `administration` → `{ grant: { role, branch_id \| null } }` — the role is the approver's choice; `guardian` → `{}`. `administration` is **Super Admin only**. Answers `{ status, account_status }`: the account turns `active` on the first approval |
| `POST` | `/admin/approvals/{id}/roles/{kind}/decline` | 🔒 | Body `{ reason }` (§5.6). Declining `guardian` rejects her pending child applications with it. The account is `rejected` only when every request was declined |
| `GET` | `/profile/role-requests` | 🔒 | **R169 §1; `held[]` since R170 §1** (the kinds she holds, from live role rows, so the screen can say why nothing is askable). The caller's own role requests (`kind`, `status`, `decided_at`) and `askable` — the kinds she may ask for now: not held, not waiting, never `guardian` (registering a child is that request). The decline reason is operator-facing and is never in this projection |
| `POST` | `/profile/role-requests` | 🔒 | **R169 §1.** An ACTIVE account asks for a further role, or again for a declined one: `student` (with consent, and a `birth_date` only where her record has none), `teaching { framing }`, or `administration { branch_id \| null }`. Opens or RE-OPENS one `RoleRequest` and notifies the approvers; grants nothing. `409` `ROLE_ALREADY_HELD` / `ALREADY_PENDING` / `BIRTH_DATE_ALREADY_RECORDED`; `400` `BIRTH_DATE_REQUIRED` / `CIRCLE_NOT_OFFERED` |
| `GET` | `/registration/circle-slots` | 🌐 | `?category_id=&branch_id=` — the memorisation circles a first-time مستفيدة may rank: the SCHEDULED weekly classes of the Category's first Level at that branch (`{ level, circles[], fixed[] }`). Anonymous like the form, so it carries a name, days and times and nothing else; never a hidden class |
...
A room carries **`capacity`** (`number | null` — *not stated*; R169 §3) on every read, and `POST` /
`PATCH` accept it as a positive whole number or `null`. It is **informational only** (BR-23, §20 rule
22): the scheduling form shows it beside the chosen room and nothing compares a roster against it.
