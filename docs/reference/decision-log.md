...
The authoritative SRS is currently through **Revision 119**. This page is a navigation index
...
| # | Date | Decision |
|---|---|---|
| **119** | 2026-09-02 | **The legal consent wording becomes versioned, immutable evidence — and the calendar filters by the scheduling-type catalogue.** *Consent:* the Arabic wording lived in the frontend's i18n catalogue and the version in a `SystemSetting` string, with a `ConsentRecord` storing only the string — **so the relationship between what a person read and what the platform recorded was human discipline**, and both drifts were silent and passed every test. `LegalConsentText` makes it a foreign key: exact wording, unique label, **immutable once in force**, **exactly one active version enforced by a partial unique index** (not a service check, which is a race), and **never deleted**. The form displays the stored wording and submits its id; a mismatch is refused, so *displayed X, recorded Y* is impossible rather than unlikely. **Legacy records are not fabricated**: pre-R119 rows keep their string and resolve to no wording, because attaching them to today's text would assert they read words nobody can prove they saw. `legal.consent_text_version` is retired from the allow-list — the stored row is left, unreadable and unwritable, as the record of what was last in force. *Scheduling:* `الجدول الزمني → النوع` filtered by the storage taxonomy (`session|event|exam`), which cannot express the association's own words — نشاط and عطلة are both an `Event`, so a holiday could not be asked for. Schedules and sittings now record their catalogue row too, the filter reads the live catalogue, and **no legacy row was backfilled**: guessing a type from a name is what §4.4b forbids |
