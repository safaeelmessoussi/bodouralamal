...
**Status:** Final MVP Blueprint (Revision 43.2 — the educational model separates *organisation* from *delivery*: Administrative Groups, Teaching Groups, Recurring Course Schedules and Sessions), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
...
* **Subject Organisation (`/admin/levels/{id}/subjects/{subjectId}`)** — where a Subject is split for a Level: create Teaching Groups and place students in them (§4.4c). **Shows the Subject's `unassigned` list for that Level** — students enrolled in the Level who hold no Teaching Group for a Subject that *is* split. A Subject with no Teaching Groups is taught to the entire Level and needs no organisation here.
...
| **Manage Teaching Groups and their membership** *(Revision 43 — organising a subject)* | ✔ | ✔ (own branches) | ⊘ | ⊘ | ⊘ | ⊘ |
