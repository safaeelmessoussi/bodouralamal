...
**Status:** Final MVP Blueprint (Revision 43.3 — the educational model separates *organisation* from *delivery*: Administrative Groups, Teaching Groups, Recurring Course Schedules and Sessions), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
...
* **Session** *(Revision 43)* — one materialized occurrence of a schedule: `schedule_id`, `date` (a `date`, never an instant — TD-11), **its own `start_time`, `end_time`, `room_id`, `teacher_id`**, `status` (lifecycle TD-1), and an `overridden` marker distinguishing a human edit from a job-generated default. Generated by `session.materialize` (TD-7). **Attendance, notes, recordings, referenced content and (post-MVP) announcements all hang here** — the Session is the occurrence that either happened or did not. Carries `version` (TD-15) plus standard audit and soft-delete fields.
...
| `session.override` *(Revision 43)* | session reference, the fields changed old→new (date/time/room/teacher). Distinguishes a human's deliberate change from a `session.materialize` default, which is exactly the distinction that protects it from the next schedule edit (§20 rule 24) |
