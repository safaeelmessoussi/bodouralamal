...
**Status:** Final MVP Blueprint (Revision 15 — `User.pre_provisioned_email` closes the pre-provisioned-account lookup gap), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
