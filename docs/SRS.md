...
**Status:** Final MVP Blueprint (Revision 25 — Category/Level scopes reserved for future revisions; user-list visibility scoped), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
...
* **Role**, **UserBranchRole** — many-to-many with branch scope; unique on `(user_id, role_id, branch_id)`, so one user may hold the same role once per branch. **`branch_id IS NULL` means ALL branches for that assignment** (Revision 24) — it is not a Super Admin marker; Super Admin's bypass follows from its role.
