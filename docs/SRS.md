...
**Status:** Final MVP Blueprint (Revision 43 — the educational model separates *organisation* from *delivery*: Administrative Groups, Teaching Groups, Recurring Course Schedules and Sessions), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
...
* **Creating a Level automatically creates its first Administrative Group, named المجموعة 1** (Revision 43), in the same transaction (TD-4). A Level with no Administrative Group is a Level nobody can be admitted to, and the approval flow (§4.1) would have nothing to offer the administrator; auto-creation means that state never exists. Further groups are created on demand (§4.4c).
...
6b. **Level creation (Revision 43, §4.4b):** `Level` insert + its first `AdministrativeGroup` (**المجموعة 1**) + `AuditLog` row. A Level that commits without its group is a Level nobody can be admitted to, and the approval screen would have nothing to offer.
...
* **Administrative Groups (Revision 43):** each seeded Level receives its first Administrative Group, **المجموعة 1**, exactly as Level creation does at runtime (§4.4b). **This is the one exception to the "no placeholder organisational data in production" rule below, and it is not a placeholder** — a Level with no group cannot accept an approved applicant, so seeding Levels without their groups would ship a system that refuses its first registration. **`branch_id` is left null-free by seeding these groups only once real branches exist** — since §15.1 forbids seeding branches, the seed creates each Level's first group **lazily, on the first branch creation**, so the invariant holds without inventing a premises. Idempotent: a Level that already has a group gains none.
