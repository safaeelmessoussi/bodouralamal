...
**Status:** Final MVP Blueprint (Revision 24 — branch-scoped authorization confirmed and made precise: per-role scope, `NULL` = all branches), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
...
* **Branch scoping is the sole authorization axis (normative, Revision 24).** A user may hold **multiple assignments of the same role**, each scoped independently; `UserBranchRole` is unique on `(user_id, role_id, branch_id)`. An assignment is scoped to **one branch, several branches (one row each), or ALL branches (`branch_id IS NULL`)**. **Super Admin bypasses branch restrictions by virtue of its role**, not by holding a null scope.
  ...
  * **Level-scoped and category-scoped role assignments are prohibited.** Category/level-based admin scoping is contemplated conceptually (§4.4b) but is **not** an authorization axis and is not to be added.
