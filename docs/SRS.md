...
**Status:** Final MVP Blueprint (Revision 43.4 — the educational model separates *organisation* from *delivery*: Administrative Groups, Teaching Groups, Recurring Course Schedules and Sessions), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
...
* **Trash restoration UI — deferred (Revision 6):** the `/admin/trash` page and its restore/permanent-delete UI are **post-MVP** (§10.1). In MVP, restoration of a soft-deleted record is performed by the developer/Super Admin via **manual SQL against the `Trash` snapshot**, following a documented runbook (restore = clear `deleted_at/deleted_by` + reinstate cascaded rows from the snapshot; **the runbook must explicitly capture and reinstate the relationship rows the TD-5 cascade removed — `FamilyLink`, `Enrollment`, `StudentTeachingGroup`, `CourseScheduleStaff` (Revision 43), `UserBranchRole`, and `UserIdentity` deactivations — a User restored without their links, enrollments and roles is a half-restored, silently broken account**; every manual restore is executed **exclusively through a locked CLI maintenance script checked into the repository** — e.g. `npm run db:restore -- --entity=User --id=<uuid>` — which wraps the snapshot restoration, the cascaded-row reinstatement, and the `trash.manual_restore` `AuditLog` insert **in a single Prisma transaction**; running restoration SQL directly in psql is prohibited, because a raw session enforces nothing and BR-15 accountability would depend on developer goodwill). The `content.quarantine-purge` job (TD-7) still enforces the 90-day permanent-delete window automatically.
...
PATCH  /admin/course-schedules/{id}   → { …, version }   DELETE /admin/course-schedules/{id}
...
1. **Unified parent+child registration (§4.1b step 5):** Parent `User` + child `User` + `FamilyLink(pending)` + `ConsentRecord`(s) + Parent `UserIdentity` + **`ConsumedToken` (onboarding `jti`)** — the token-consumption insert inside this transaction is the replay guard (§4.1b). **The applicant's `intended_branch_id` is a column on the parent `User` row written by this same transaction (Revision 39), not an additional write** — and it is written on the applicant only, never copied onto the child.
...
| `session.regenerate` *(Revision 43.4)* | session reference, the date range, and **what it overwrote** — the room and staff as they stood before. The one sanctioned path by which a Session that has already happened is brought back into line with its schedule; it exists so that act is deliberate and attributable rather than a side effect of an edit |
