...
**Status:** Final MVP Blueprint (Revision 26 — reference/configuration data is Super Admin only; operational data is branch-scoped Admin), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
