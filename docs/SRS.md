...
**Status:** Final MVP Blueprint (Revision 39 — registration captures the applicant's chosen Branch; placement into a Group remains an administrative action after approval), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
