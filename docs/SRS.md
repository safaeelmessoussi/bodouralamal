...
**Status:** Final MVP Blueprint (Revision 38 — the API contract is an intentional interface: explicit DTOs, never serialised ORM models), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
...
* **Registration never places a beneficiary (normative, Revision 29).** Applicants **never** select Branch, Room, Level or Group during self-registration; **registration creates a pending applicant only**, and **assignment to Branch, Room, Level and Group is an administrative action performed after approval**. Registration metadata — the field definitions, registration profiles, required fields and onboarding rules that **render and validate** the form — **must not expose or duplicate reference/configuration data**; Categories, Levels, Branches, Rooms, Subjects and AcademicYear remain behind their own reference-data APIs under Revision 26's permission split.
