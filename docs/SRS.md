...
* **Rate Limiting:** Managed at the **Nginx reverse-proxy layer** (login/API burst limits, values in TD-13), not in-process memory — limits survive container restarts and remain correct if the API is ever scaled to multiple containers.
...
Exact patch versions are locked by `package-lock.json` and the Docker image digests. **Major versions are locked at project initialization (M0):** "latest stable" is evaluated exactly once, when the lockfiles are first created — after that, any upgrade (major, minor, or patch beyond lockfile refresh) happens only as an explicit, dedicated implementation task recorded in `docs/CHANGES.log`, never as a side effect of another task and never on an agent's initiative mid-build.
...
* Soft-deleted/deactivated accounts show "Account deactivated — contact your branch admin" on login attempt. Never silently re-register or reactivate.
...
3. The server looks up `UserIdentity (google, provider_subject_id)`, then falls back to matching a pre-provisioned account by verified email.
4. Routing decision:
   * **4a — Identity exists:** establish session; route by `account_status` (TD-1): `Active` → role dashboard; `Pending` → status screen; `Suspended`/deactivated → "Account deactivated" screen.
   * **4b — No identity, but a staff pre-provisioned account matches the verified email:** bind the Google identity to that account (create the `UserIdentity` row), then route by its `account_status` (typically `Pending` → status screen, or `Active` if staff pre-approved).
...
* **Pass/fail:** default minimum passing grade configurable per level (association default: 10). **Teachers/Admins can manually override pass/fail per student**, stored separately with actor/timestamp capture in the `AuditLog` (TD-8).
...
* **UserIdentity** — provider-abstracted auth linkage: `(user_id, provider, provider_subject_id, email)`. MVP: `provider = google` only. Unique on `(provider, provider_subject_id)` (TD-6). Created atomically with the User in self-service registration (§4.1b step 5) or bound on first login for pre-provisioned accounts (§4.1b step 4b).
...
* **AuditLog** — `(actor_user_id, action_type, target_entity, target_id, detail JSONB [incl. mandatory justifications], created_at)`; coverage grid TD-8.
...
10. **Pre-provisioned identity binding (§4.1b step 4b):** `UserIdentity` creation + any status-driven side effects, atomic with the login routing decision.
...
| User | **Soft-delete only** (`deleted_at = now()`): anonymize sensitive profile fields (phone, notes, social-profile fields) in the live row (full snapshot preserved in `Trash`), deactivate `UserIdentity` rows, and cascade-remove `FamilyLink` and `GroupTeacher` rows. Grades/QuranProgressLog rows are retained (historical record) but detached from directories. |
...
| `token.purge` | cron (daily) | — | deletes `ConsumedToken` rows past their TTL horizon (keeps the replay-guard table small) |
...
* **Student search** (`GET /admin/users?role=student&q=…`) matches across: Arabic name, French name, nickname, phone, linked parent's name, and Google email. **Search semantics (normative, Revision 9):**
...
* **Email normalization:** every Google OAuth email is **lowercased before every lookup and every write** — identity binding (§4.1b step 4b), pre-provision matching, `SUPER_ADMIN_EMAIL` comparison, and `UserIdentity` persistence all operate on the lowercased form, so case variants of one mailbox can never create or match distinct identities.
...
* **SystemSetting (application-level, runtime-editable):** branding assets, legal/consent text versions, Hijri offset, Category default visibilities, per-level passing grade defaults.
* **Nginx rate limits (starting values, tunable):** auth endpoints 10 req/min/IP; general API 120 req/min/IP; upload initiations 30/hour/user. `/storage/` location: `client_max_body_size 110m`, `proxy_request_buffering off`; `/api/v1/`: `client_max_body_size 2m` (§3.1).
...
* **Super Admin:** 1 `User` (status `active`) pre-provisioned against the `SUPER_ADMIN_EMAIL` env var (TD-13); identity binds on first Google login (§4.1b step 4b). No password anywhere.
...
* **SystemSetting defaults:** Hijri offset 0; Category default visibility — Children `private`, Teens `private`, Women `public`; per-level passing grade 10.
...
**Platform & Deployment**
☑ §19.1 pipeline from clean VPS to healthy /healthz ☑ §19.0 topology respected (fixtures-only outside Morocco — checked in fixtures guard) ☑ hand-written migration SQL present in history (CI check) ☑ `db push` absent from all scripts ☑ backup job + restore drill under RTO ☑ rate limits active at Nginx ☑ same-origin path routing serves client, API, and storage under one domain ☑ no PII in logs (log audit) ☑ tests pass.
