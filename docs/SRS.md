...
**Status:** Final MVP Blueprint (Revision 31 — the official Moroccan Hijri calendar is the source of truth; `HijriMonthStart` replaces the global day-offset), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
...
| **Enter the official Moroccan Hijri calendar for the launch year and publish it (Revision 31)** | The overlay reproduces the Ministry of Habous publication and **computes nothing**: until a month is recorded and published, dates in it render with no Hijri label. The Ministry announces each month only **after** the sighting on the evening of the 29th, so this is a recurring monthly task, not a one-off — roughly one entry a month, plus any correction | Recurring admin work |
...
* **Hijri Calendar Management (`/superadmin/hijri-calendar`)** — *(Revision 31)* the maintenance surface for the official Moroccan Hijri calendar. A **year selector**; the **twelve Hijri months** of the selected year, each with its **Gregorian start date**; and the actions **Edit**, **Save**, **Publish**, **View history**, and **Import official calendar**. Only **published** months are rendered anywhere in the platform, so a year can be entered progressively and reviewed before it becomes visible. **Import official calendar** is an integration point, not a completed importer (Revision 31): it invokes an import service interface a future authoritative Ministry source can implement, and until one exists it offers an upload workflow or reports *not yet configured*. **Manual entry and automated import populate the same `HijriMonthStart` table** — there is no second path.
...
* **Automated Hijri Calendar Import (Revision 31 integration point):** the MVP ships the import *interface* and the `POST /admin/hijri-calendar/import` route, which writes `HijriMonthStart` like any manual edit. What is post-MVP is a concrete adapter behind it, because no authoritative machine-readable source exists today: the Ministry publishes each month start as a prose news announcement with no API, feed or dataset, and since Morocco fixes months by observation a year cannot be published in advance. If the Ministry later offers a structured publication, an adapter implements the existing interface and **no calendar-reading code changes** — every consumer already reads `HijriMonthStart` and nothing else.
...
| **Manage the official Hijri calendar (`HijriMonthStart`) — edit, publish, import** *(Revision 31)* | ✔ | ⊘ | ⊘ | ⊘ | ⊘ | ⊘ |
...
POST  /admin/hijri-calendar/import           → the Revision-31 integration point: invokes the import
                                               service interface and writes HijriMonthStart. Reports
                                               NOT_CONFIGURED while no authoritative source is wired.
```
...
| `hijri.month_start.set` *(Revision 31)* | `hijri_year`, `hijri_month`, the previous and new `gregorian_start_date` (`null` previous on first entry), and the `version` written. Both values are recorded because the *correction* is the interesting event: this table reproduces an official publication, and a wrong month start silently shifts every Hijri label in it |
| `hijri.year.publish` *(Revision 31)* | `hijri_year` and the count of months moved from `draft` to `published`. Publishing is what makes a month visible platform-wide, so it is a distinct event from editing it |
| `hijri.import` *(Revision 31)* | the source identifier, the number of months written, and the outcome. Recorded even when the import is refused as not-configured, so an attempt to populate the official calendar is never invisible |
