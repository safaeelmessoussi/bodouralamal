...
* Teacher-to-group assignment (including co-teaching) is stored in the **`GroupTeacher`** join table (§7); all teacher scoping (exam authoring, Quran logging, sensitive social-data access §4.10) resolves **exclusively** through it. A Teacher's `UserBranchRole` row carries the role, **not** their teaching reach — a Teacher teaching Level 1 in Marrakesh and Level 2 in Casablanca is expressed by their `GroupTeacher` rows, since a `Group` carries both `level_id` and `branch_id` (Revision 24).
...
* **Registration → Access:** Landing → Continue with Google (§4.1b) → unified/adult form (read-only email, **applicant chooses a Branch — R39**) → Pending status screen → [Admin approval] → **[Admin assigns a Group]** → Login → role-based dashboard redirect. **The applicant chooses only the Branch; Level, Room and Group are administrative decisions taken after approval.**
...
14. Never store Group/Event schedule times as UTC instants (TD-11) — Morocco's Ramadan DST suspension will corrupt every weekly schedule. Never descope the Hijri overlay (§4.4 — in-scope by explicit decision), and **never compute a Hijri date astronomically or reintroduce a global day-offset** (Revision 31): every Hijri value is read from `HijriMonthStart`, through `baseHijri()`, and a month that is not recorded and published renders nothing. **The Super Admin records the Ministry's official announcement and does not decide it** (Revision 32) — user-facing and code vocabulary is *record* / *official announcement*, never *choose*, *define* or *set*.
