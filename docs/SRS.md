...
**Status:** Final MVP Blueprint (Revision 43.1 — the educational model separates *organisation* from *delivery*: Administrative Groups, Teaching Groups, Recurring Course Schedules and Sessions), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
...
* **StudentTeachingGroup** *(Revision 43)* — join: Student↔TeachingGroup. **At most one row per (student, subject, level) among non-deleted rows** (TD-6); membership is optional, and an enrolled student holding none for a split Subject appears on that Subject's unassigned list (§5.6).
...
6a. **Teaching Group membership mutation (Revision 43):** `StudentTeachingGroup` insert/delete + consent re-evaluation enqueue for the affected teaching group's sessions. The "at most one per (student, subject, level)" rule is a partial unique index (TD-6), so it too is declarative rather than locked.
...
* **`StudentTeachingGroup` — at most one per `(student_id, subject_id, level_id)` among non-deleted rows** *(Revision 43)*. `subject_id` and `level_id` come from the teaching group, so this is a **functional unique index over the join** (TD-6a hand-written SQL), not a plain column index.
