...
**Status:** Final MVP Blueprint (Revision 19 — `audit.purge` bounded by an enumerated action-type allowlist, never by age alone), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
