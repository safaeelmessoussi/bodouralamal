...
**Status:** Final MVP Blueprint (Revision 22 — `SUPER_ADMIN_EMAIL` is a bootstrap-only value; the database is authoritative for administrators after bootstrap), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
...
| `SUPER_ADMIN_EMAIL` | Yes | — | `admin@bodour.ma` | **Bootstrap-only** Super Admin seed (§15.1, Revision 22): consumed by `seed:production`, **never by the running API**, and **ignored permanently once an active Super Administrator exists**. Still Required — the first deployment cannot complete without it (§19.1 step 2) |
...
2. cp .env.example .env        # fill TD-13 values; SUPER_ADMIN_EMAIL mandatory (bootstrap only, §15.1 R22)
