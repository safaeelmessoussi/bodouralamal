...
**Status:** Final MVP Blueprint (Revision 23 — `SUPER_ADMIN_EMAIL` conditionally required; linking an existing child is staff-mediated), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
