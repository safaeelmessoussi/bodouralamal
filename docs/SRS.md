...
**Status:** Final MVP Blueprint (Revision 16 — session layer made implementable: `RefreshToken`, refresh route, rotation/revocation transactions, terminal-status routing, family-link revocation), **immutable source of truth** — changed only by an explicit Document Owner revision, never by an implementing agent
...
| `auth.token_revoked` *(Revision 16)* | reason (`suspension` \| `user_deleted` \| `reuse_detected`), sessions affected count (TD-4.15). A `reuse_detected` row is a **security event**: it means a rotated token was replayed outside the grace window |
