...
- [x] Rotation / logout / revoke-on-suspension transactions (TD-4.13/14/15); logout revocation + `auth.logout` audit are atomic; rotation and logout serialize per `session_id`; 10 s grace is idempotent (no chain fork) and cannot resurrect a logged-out chain
