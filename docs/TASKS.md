...
- [x] Upload initiate/complete/abort: single-shot presigned PUT, branch-scope validation, Teacher Global rejection (§4.9, TD-3.5)
  ...
  - ✓ **B-03 immutable finalization:** the presigned PUT targets `staging/content/...`; ETag-bound server promotion creates a distinct canonical key never client-writable; transactional audit publication, replacement compare-and-swap, idempotent retry/restart and legacy-ticket safety are proven against real PostgreSQL/MinIO. `upload.gc` remains the separate abandoned-object collector
...
- [x] Hash-segmented immutable canonical keys; clients write staging only, completion promotes one verified ETag, and replacement mints a new key + quarantines old (TD-9)
