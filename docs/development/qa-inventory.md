...
| `verify-teacher-scheduling.sh` | 22 | **The merged مؤطرة surface** — one node, the calendar+table page, a responsible selector offering only her, an activity she creates end to end with an assistant (asserted from the network layer), **R93's assignment notice in the assistant's own bell**, and **R94's type picker** — نشاط + امتحان, never حصة, with an exam saved against one of her own classes | **STALE — 4/14** (found 2026-09-20). It selects the item type by the literal value `class`, and since R110 the picker's values are catalogue ids, so the class form it means to inspect never opens and most later checks inherit that. Unrelated to R163; its 13a assertion was restated (she is asked no «نمط التدريس» and offered none of the administrator's filters) but cannot be reached until the type step selects by the option's text |
...
| `verify-schedule-edit.sh` | 10, 11 | «تعديل العنصر» — R50's scopes through the real dialog | **STALE — 6 checks pass, then it throws** (found 2026-09-20). It sets «نهاية التكرار» through a native `input[type="date"]`, and every native date input was retired for the platform's own picker, so its selector finds nothing. Unrelated to R163; its mode assertion was restated (no «نمط التدريس» is rendered) and passes. Needs its date step rewritten on `.date-picker__trigger`, as `verify-exam-scheduling.mjs` already does |
...
| `verify-class-filters.sh` | 10 | **R163 §5** — a class addressed through the five filters on the real إضافة عنصر form: no «نمط التدريس», every filter «الكل», a Category narrowing the Levels on offer, one branch answering «الفرع المنظِّم», Level/group/circle all at «الكل» refused in words, the saved row filter-built with the list naming its audience rather than «أبعاد متعددة»; then the «from this date onward» editor — same filters, opened on the class as it stands, Subject kept, a missing year named on screen, and a real split whose successor is filter-built while the predecessor keeps its own target | **12/12** (2026-09-20) |
