...
| 25 | Trash | `/admin/trash` | ✓ | ✓ `trash`, `trash-coverage`, `trash-lifecycle` | ✓ `verify-trash-restore` (R169 §8 — restore only) | purge is still un-driven in a browser |
...
| `verify-registration.sh` | 13 | The FAMILY registration journey end to end — «أسجّل أبنائي» chosen in the closed role control after UNTICKING its default «أسجّل نفسي كمستفيدة» (R168 §1, R170 §2; `role-chooser.mjs`), each person's ONE optional block (pair rule said once, nothing required inside or below it), two children with their own dates (chosen on the real calendar via `date-picker.mjs`), branches, Categories and media answers — plus the records it must and must not create | 45/45 + 12/12 DB |
| `verify-role-requests.sh` | 10 | **R168 §1 on the real pages** (scenario: `seed-role-requests-scenario.ts`, three SCHEDULED memorisation حلقات at a Category's first Level). One applicant ticks all four choices and is asked who she is once («بيانات ولي الأمر» gone the moment an adult role is ticked); the circles offered are the scheduled ones with their days and times, her order is hers to change, the whole-Level class is shown as no choice, and the payload carries four roles and no administrative role name. A branch-less **Admin** sees every role with its own state, gets the per-role review instead of «موافقة»/«رفض», is SHOWN the administration request and offered no decision on it, approves teaching (grant offers مؤطِّرة only, no «الموافقة دون دور») — the first approval activates the account — and declines guardian with a reason. The **Super Admin** places her with her ranked circles beside the control (first Level preselected), and for administration chooses between مسؤولة and مشرفة عامة, the latter stating its scope instead of asking; the item then leaves the queue. **R169 §1:** an account that already exists is offered only what it does not hold, asks for هيئة الإدارة, appears in the queue with the per-role review ONLY (no whole-account decision), is declined without her account being touched, sees «مرفوض» on «حسابي» and never the reason, asks AGAIN, and the request is one pending line. **R170:** «ماذا تريدين؟» is CLOSED and names its default, opening it offers four with one ticked, the default unticks, several are ticked at once; and an account holding EVERY askable role still finds «صفاتي وطلباتي», its roles named and the missing button explained | 85/85 |
| `verify-whole-category-recording.sh` | 7 | **«تسجيل صوتي» for a whole Category (R172 §1/§11).** The recorder asks for the Category first; «كل مستويات الفئة» is in the Level list and stays chosen; with nothing assigned whole it says so and names «مواد المستوى»; a Subject assigned there through the real dialog is then offered with no Level and the recorder is ready | 7/7 |
| `verify-level-subjects.sh` | 7 | **«مواد المستوى» (Staging, 2026-09-23).** The page makes no per-Level or per-Category read and a bounded number of reads on load; every Level lists its Subjects (never «no subjects» from a refused read); editing a Level that keeps an assigned Subject and adds one saves without `DUPLICATE` | 7/7 |
| `verify-operations-status.sh` | 10 | **R169 §11 — «حالة النظام» on the real page.** Refused anonymously; found in the Super Admin's menu, last in الإدارة; five counts, each saying what it MEANS; the headline agreeing with the numbers; the sentence about what it cannot see (backup, certificate); and no payload, key or error text on the screen | 8/8 |
| `verify-trash-restore.sh` | 10 | **R169 §8 — the first browser check of the Trash.** A Subject circle deleted through the real endpoint is listed, is OFFERED «استعادة» (it used to read «غير متاح — يتبعه سجلات أخرى»), asks before restoring, comes back, SAYS how many seats returned with it, and leaves the Trash | 6/6 |
| `verify-room-capacity.sh` | 10 | **R169 §3** — the rooms dialog refuses a non-whole number in words before the wire, accepts a capacity, says it back in the list, keeps it across a reload, hydrates it when editing and saves «not stated» when cleared | 8/8 |
...
| `verify-occurrence-details.sh` | 23 | **AT** — the one details dialog opened from all four calendars on a real Session; two content sections; no page step; every focused read a 200. **Stale from 2026-09-01 to 2026-09-22**: its scenario's one occurrence is dated 2026-08-24 and the personal calendars open at today's month; it now walks back to the scenario's month by the nav's ARIA label | 13/13 |
...
| `verify-circle-branch.sh` | 10 | R172 §15 — a circle is created in a branch; the list shows and filters by it | 10/10 |
| `verify-weekly-follows-start.sh` | 4 | R172 §13 — a weekly class follows its start date on «تعديل» | 4/4 |
