...
| `verify-teacher-scheduling.sh` | 22 | **The merged مؤطرة surface** — one node, the calendar+table page, a responsible selector offering only her, an activity she creates end to end with an assistant (asserted from the network layer), and **R93's assignment notice in the assistant's own bell** | 9/9 |
