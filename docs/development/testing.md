...
| `scripts/dev/browser/verify-unsaved-guard.sh` | Rule AY in a browser, both halves: `＋إضافة مقر` **pristine** closes on a backdrop click with no question; **dirty** refuses the backdrop, asks on Escape and on Cancel, keeps the typed value when the reader continues editing, becomes **pristine again** when the value is restored, and closes only on an explicit discard. `＋تسجيل مستفيدة` is re-run as the reference. **Last run: 21/21.** |
