...
| `scripts/dev/browser/verify-public-reader.mjs` (owned by `scripts/ci/test-integration.sh`) | Required disposable-stack proof of the public reader and canonical occurrence dialog: a fresh cookieless profile loads public image/PDF/audio/video and download-only Office-document bytes through Nginx, decodes/plays inline media, follows a content back-link to `/calendar`, reloads the stable link, and rejects stale/private coordinates. Synthetic Admin/Teacher/Student sessions prove authenticated private success versus unauthorized denial. It checks the responsive calendar/dialog, operated phone filters, and non-overlapping official Hijri/Gregorian/time labels at 320, 360, 375, 390, 412, 430, 768 and 1280 px without turning this bounded regression into an unrelated audit of every back-office table. Chrome and ffmpeg are required by the CI integration gate; the test cannot silently skip there. GitHub's hosted Ubuntu image supplies Chrome/Chromium and the workflow installs the otherwise-absent ffmpeg package explicitly. |
