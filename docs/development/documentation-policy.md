...
| A **cross-cutting UI rule** — a shared component's contract, a page-shape convention, a labelling or wording rule that applies platform-wide | [Platform UX & atomic design](ux-architecture.md). **This is the one page a UI change most often needs and most often skips**: a rule recorded only in the screen that first needed it is a rule the next screen implements differently, which is how this platform came to hold two button systems and three copies of the Level label |
