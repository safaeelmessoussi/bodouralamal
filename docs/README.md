...
| [`SRS-PROPOSAL-R110.md`](SRS-PROPOSAL-R110.md) | **Draft revision awaiting the Document Owner** — the scheduling-type catalogue becomes managed reference data, and an activity records its type. **The behaviour is already live** (NEW H); this proposes only the normative wording, and explains why it exercises R56's own clause rather than contradicting it | Contributors may draft; only the Owner applies |
