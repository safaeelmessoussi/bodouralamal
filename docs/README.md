...
| [`SRS-PROPOSAL-R51.md`](SRS-PROPOSAL-R51.md) | **Draft revision awaiting the Document Owner** — Scheduling becomes one navigation section with Events and Sessions under it. §14.1's sitemap is normative, so the regrouping cannot ship without it; the component sharing that accompanies it needed no revision and is already done | Contributors may draft; only the Owner applies |
