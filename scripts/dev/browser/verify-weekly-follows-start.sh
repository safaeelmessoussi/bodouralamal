#!/usr/bin/env bash
# R172 §13 — a plain weekly class follows its start date on «تعديل», on the real
# «الجدول» page (verify-weekly-follows-start.mjs), against the dev scenario.
set -euo pipefail
...
for _ in $(seq 1 60); do
  curl -sf http://127.0.0.1:9231/json/list >/dev/null 2>&1 && { CHROME_READY=1; break; }
  sleep 0.5
done
if [[ "$CHROME_READY" != "1" ]]; then
  echo "FAIL: Chrome never opened its debug port on 9231"
  exit 1
fi

PORT=9231 node scripts/dev/browser/verify-weekly-follows-start.mjs
