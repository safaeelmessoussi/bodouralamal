#!/usr/bin/env bash
# «تسجيل صوتي» for a whole Category: the choice, the hint, the assignment, the recorder ready.
#
...
for _ in $(seq 1 60); do
  curl -sf http://127.0.0.1:9233/json/list >/dev/null 2>&1 && { CHROME_READY=1; break; }
  sleep 0.5
done
if [[ "$CHROME_READY" != "1" ]]; then
  echo "FAIL: Chrome never opened its debug port on 9233"
  exit 1
fi

PORT=9233 node scripts/dev/browser/verify-whole-category-recording.mjs
