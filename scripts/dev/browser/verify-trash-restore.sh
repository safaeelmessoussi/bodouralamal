#!/usr/bin/env bash
# SRS Revision 169 §8 — restoring a Subject circle on the real Trash screen. See the .mjs.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

CHROME="$(command -v google-chrome || command -v chromium || command -v chromium-browser || true)"
[[ -n "$CHROME" ]] || { echo "SKIP: no Chrome on this machine; the Trash restore was not verified"; exit 0; }
...
for _ in $(seq 1 60); do
  curl -sf http://127.0.0.1:9270/json/list >/dev/null 2>&1 && { READY=1; break; }
  sleep 0.5
done
[[ "$READY" == "1" ]] || { echo "FAIL: Chrome never opened its debug port on 9270"; exit 1; }

PORT=9270 node scripts/dev/browser/verify-trash-restore.mjs
