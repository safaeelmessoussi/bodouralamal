...
for _ in $(seq 1 60); do
  curl -sf http://127.0.0.1:9227/json/list >/dev/null 2>&1 && { CHROME_READY=1; break; }
  sleep 0.5
done
if [[ "$CHROME_READY" != "1" ]]; then
  echo "FAIL: Chrome never opened its debug port on 9227"
  exit 1
fi

PORT=9227 node scripts/dev/browser/verify-library-recorder.mjs
