#!/usr/bin/env bash
# R90 — the staff picker's planning warnings, through the real scheduling form.
set -euo pipefail
...
export R90_SCENARIO="$(cd backend && npx tsx scripts/seed-r90-scenario.ts | tail -1)"
...
cleanup() {
  ...
  (cd backend && npx tsx scripts/seed-r90-scenario.ts --clean) >/dev/null 2>&1 || true
}
trap cleanup EXIT
...
for _ in $(seq 1 60); do
  curl -sf http://127.0.0.1:9243/json/list >/dev/null 2>&1 && { CHROME_READY=1; break; }
  sleep 0.5
done
if [[ "$CHROME_READY" != "1" ]]; then
  echo "FAIL: Chrome never opened its debug port on 9243"
  exit 1
fi

PORT=9243 node scripts/dev/browser/verify-staff-picker.mjs
