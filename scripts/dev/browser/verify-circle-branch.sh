#!/usr/bin/env bash
# R172 §15 — «إضافة حلقة» asks for الفرع; «حلقات المواد» shows and filters by
# it, on the real page (verify-circle-branch.mjs), against the dev scenario.
set -euo pipefail
...
for _ in $(seq 1 60); do
  curl -sf http://127.0.0.1:9229/json/list >/dev/null 2>&1 && { CHROME_READY=1; break; }
  sleep 0.5
done
if [[ "$CHROME_READY" != "1" ]]; then
  echo "FAIL: Chrome never opened its debug port on 9229"
  exit 1
fi

PORT=9229 node scripts/dev/browser/verify-circle-branch.mjs
